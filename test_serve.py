...
class ResolverTests(unittest.TestCase):
    ...
    def test_gold_yield_request_is_clean_and_native_dual_axis(self):
        prompt = "10 yr yield versus price of gold on opposite axis over the last 10 years"
        contract = serve.parse_macro_request_contract(prompt)
        self.assertEqual(
            [operand["sourceSpan"] for operand in contract["operands"]],
            ["10 yr yield", "price of gold"],
        )
        self.assertEqual(contract["time"]["qualifiers"][0]["value"], 10.0)
        self.assertEqual(contract["presentation"]["axis"]["mode"], "dual")
        self.assertEqual(contract["presentation"]["transform"]["mode"], "raw")
        selected, notices = serve.resolve_prompt_series(prompt, allow_fred_search=False)
        self.assertEqual([row["id"] for row in selected], ["DGS10", "GOLD_PRICE"])
        self.assertEqual(notices, [])
        config = serve.parse_chart_intent(prompt, selected, contract)
        self.assertEqual(
            {row["id"]: (row["axis"], row["units"]) for row in config["series"]},
            {"DGS10": ("left", "raw"), "GOLD_PRICE": ("right", "raw")},
        )
        self.assertEqual(config["axes"]["left"]["title"], "Percent")
        self.assertEqual(config["axes"]["right"]["title"], "U.S. dollars per troy ounce")

    def test_unit_aware_default_and_same_axis_override(self):
        selected, _ = serve.resolve_prompt_series(
            "10yr yield versus price of gold over the last 10 years",
            allow_fred_search=False,
        )
        automatic = serve.parse_chart_intent(
            "10yr yield versus price of gold over the last 10 years",
            selected,
        )
        self.assertEqual({row["axis"] for row in automatic["series"]}, {"left", "right"})
        self.assertEqual({row["units"] for row in automatic["series"]}, {"raw"})
        same_axis = serve.parse_chart_intent(
            "10yr yield versus price of gold on the same axis",
            selected,
        )
        self.assertEqual({row["axis"] for row in same_axis["series"]}, {"left"})
        self.assertEqual({row["units"] for row in same_axis["series"]}, {"raw"})

    def test_axis_phrases_are_fully_consumed_and_ordinal_dual_is_stable(self):
        secondary = "10-year Treasury yield versus 2-year Treasury yield over the last 1 year on a secondary axis"
        contract = serve.parse_macro_request_contract(secondary)
        self.assertEqual(
            [operand["sourceSpan"] for operand in contract["operands"]],
            ["10-year treasury yield", "2-year treasury yield"],
        )
        self.assertEqual(contract["presentation"]["axis"]["mode"], "dual")
        selected, _ = serve.resolve_prompt_series(secondary, allow_fred_search=False)
        config = serve.parse_chart_intent(secondary, selected, contract)
        self.assertEqual(
            {row["id"]: row["axis"] for row in config["series"]},
            {"DGS10": "left", "DGS2": "right"},
        )

        separate = serve.parse_macro_request_contract(
            "10-year Treasury yield versus gold price on separate axes"
        )
        self.assertEqual(
            [operand["sourceSpan"] for operand in separate["operands"]],
            ["10-year treasury yield", "gold price"],
        )
        self.assertEqual(separate["presentation"]["axis"]["mode"], "dual")

        single = serve.parse_macro_request_contract(
            "10-year Treasury yield versus 2-year Treasury yield over the last 1 year on one scale"
        )
        self.assertEqual(
            [operand["sourceSpan"] for operand in single["operands"]],
            ["10-year treasury yield", "2-year treasury yield"],
        )
        self.assertEqual(single["presentation"]["axis"]["mode"], "single")

        ordinal = serve.parse_macro_request_contract(
            "10-year Treasury yield versus 2-year Treasury yield over the last 1 year "
            "put the first series on the left axis and the second on the right axis"
        )
        ordinal_config = serve.parse_chart_intent(ordinal["conceptText"], selected, ordinal)
        self.assertEqual(
            {row["id"]: row["axis"] for row in ordinal_config["series"]},
            {"DGS10": "left", "DGS2": "right"},
        )

    def test_presentation_wrappers_and_ordered_colors_do_not_leak(self):
        prompt = (
            "graph 10-year Treasury yield versus 2-year Treasury yield over the last 10 years "
            "in native levels as a line chart with the first series blue and the second orange"
        )
        contract = serve.parse_macro_request_contract(prompt)
        self.assertEqual(
            [operand["sourceSpan"] for operand in contract["operands"]],
        ...
        self.assertEqual(contract["presentation"]["transform"]["mode"], "raw")
        self.assertEqual(contract["presentation"]["chartType"], "line")
        self.assertEqual(contract["presentation"]["style"]["colors"], ["blue", "orange"])
        selected, _ = serve.resolve_prompt_series(prompt, allow_fred_search=False)
        config = serve.parse_chart_intent(prompt, [*selected, selected[0]], contract)
        self.assertEqual(
            [row["color"] for row in config["series"]],
            ["#0b65c2", "#f59e0b", "#0b65c2"],
        )

        hex_prompt = (
            "graph 10-year Treasury yield versus 2-year Treasury yield using #3366cc for the "
            "first series and #cc6633 for the second"
        )
        hex_contract = serve.parse_macro_request_contract(hex_prompt)
        self.assertEqual(hex_contract["presentation"]["style"]["colors"], ["#3366cc", "#cc6633"])
        self.assertEqual(
            [operand["sourceSpan"] for operand in hex_contract["operands"]],
            ["10-year treasury yield", "2-year treasury yield"],
        )

    def test_unsupported_chart_style_builds_with_assumptions_not_unresolved_data(self):
        prompt = "chart gold price versus 10 yr yield in a candlestick chart over the last 10 years"
        selected, _ = serve.resolve_prompt_series(prompt, allow_fred_search=False)
        ...
        def fake_series(entry, _start, _end):
            ...
            return {
                ...
                "sourceUrl": "https://example.test/source", "observations": observations,
                "firstDate": observations[0]["date"], "lastDate": observations[-1]["date"],
                "latest": observations[-1]["value"], "resolution": "fixture",
            }
        ...
        self.assertEqual(payload["presentationReview"]["unsupported"], ["in a candlestick chart"])
        self.assertTrue(payload["presentationReview"]["builtWithAssumptions"])
        self.assertTrue(any("built with safe assumptions" in notice for notice in payload["resolutionNotices"]))
    ...
    def test_model_gate_rejects_invented_specificity(self):
        unsafe_pairs = (
            ("CPI", "core CPI"),
            ("GDP", "real GDP"),
            ("S&P 500 index", "S&P 500 operating earnings per share"),
        )
        for source, canonical in unsafe_pairs:
            with self.subTest(source=source, canonical=canonical):
                safe, reason = serve.macro_model_mapping_is_safe(source, canonical)
    ...
    def test_auto_axes_use_native_measurement_signatures(self):
        cases = (
            ("S&P 500 versus VIX", [serve.CATALOG_BY_ID["SP500"], serve.CATALOG_BY_ID["VIXCLS"]]),
            ("gold price versus natural gas price", [serve.CATALOG_BY_ID["GOLD_PRICE"], serve.CATALOG_BY_ID["DHHNGSP"]]),
        )
        for prompt, selected in cases:
            with self.subTest(prompt=prompt):
                config = serve.parse_chart_intent(prompt, selected)
                self.assertEqual([row["axis"] for row in config["series"]], ["left", "right"])

    def test_all_comparison_connectors_resolve_each_operand_independently(self):
        for connector in ("versus", "vs.", "against", "compared with", "compared to", "and"):
            prompt = f"30-year Treasury yield {connector} 10-year real Treasury yield"
            with self.subTest(connector=connector):
                selected, _ = serve.resolve_prompt_series(prompt, allow_fred_search=False)
                self.assertEqual([row["id"] for row in selected], ["DGS30", "DFII10"])

    def test_price_transform_clarification_updates_the_price_series_axis_title(self):
        selected = [serve.CATALOG_BY_ID["DGS10"], serve.CATALOG_BY_ID["CPIAUCSL"]]
        config = serve.parse_chart_intent("10-year yield versus CPI", selected)
        self.assertEqual([row["axis"] for row in config["series"]], ["left", "right"])
        serve.apply_macro_chart_clarifications(config, {"price_transform": "pct_yoy"})
        self.assertEqual(config["axes"]["left"]["title"], "Percent")
        self.assertEqual(config["axes"]["right"]["title"], "Percent change from year ago")
    ...
    def test_macro_model_fallback_is_re_resolved_and_never_supplies_values(self):
        ...
        self.assertEqual(payload["intentResolution"]["modelRouting"]["status"], "accepted")
        self.assertTrue(payload["intentResolution"]["modelRouting"]["attempted"])
        self.assertEqual(payload["intentResolution"]["modelRouting"]["accepted"], ["unemployment rate"])
        self.assertEqual(payload["intentResolution"]["modelRouting"]["inputConcepts"], ["labor slackness gauge"])
