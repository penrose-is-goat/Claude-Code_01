...
function refreshAxisTitles() {
  ["left", "right"].forEach((side) => {
    const labels = [...new Set(state.rows
      .filter((row) => row.config.visible && row.config.axis === side)
      .map((row) => unitsLabel(row.config, row.series))
      .filter(Boolean))];
    state.axes[side].title = labels.join(" / ");
  });
}
...
function renderAll() {
  ...
  const unsupported = state.payload.presentationReview?.unsupported ||
    state.payload.intentResolution?.requestContract?.presentation?.unsupported || [];
  const layoutMessage = recognized.length
    ? `<strong>Prompt layout applied:</strong> ${recognized.map(escapeHtml).join(" ")}`
    : "<strong>Layout:</strong> Automatic unit-aware defaults applied; use Edit graph to assign axes and styles.";
  instructionAudit.innerHTML = `${layoutMessage}${unsupported.length
    ? ` <span class="instruction-assumption"><strong>Build with assumptions:</strong> ${unsupported.map(escapeHtml).join("; ")} was not a supported chart style. <button id="reviewPresentationBtn" class="data-button ghost" type="button">Edit chart settings</button></span>`
    : ""}`;
  const reviewPresentationBtn = $("#reviewPresentationBtn");
  if (reviewPresentationBtn) {
    reviewPresentationBtn.addEventListener("click", () => graphEditor.classList.remove("hidden"));
  }
...
async function addSeries(clarifications = null) {
  ...
  try {
    ...
    state.payload.sources = [...(state.payload.sources || []), ...(payload.sources || [])];
    state.payload.chartConfig.series = state.rows.map((row) => ({ ...row.config }));
    state.payload.chartConfig.recognizedInstructions = [...new Set([
      ...(state.payload.chartConfig.recognizedInstructions || []),
      ...(payload.chartConfig?.recognizedInstructions || []),
    ])];
    refreshAxisTitles();
...
seriesEditor.addEventListener("change", (event) => {
  ...
  if (field === "axis" || field === "units" || field === "visible") refreshAxisTitles();
