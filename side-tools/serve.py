...
def resolve_series(prompt: str) -> list[dict[str, Any]]:
    ...
    clauses = [
        clause.strip()
        for clause in re.split(r",|;|\band\b|\bversus\b|\bvs\.?\b", normalized)
        if clause.strip()
    ] or [normalized]
    ...
    if re.search(r"\bbreakeven(?: inflation)?\b", normalized):
        for tenor, series_id in (("5", "T5YIE"), ("10", "T10YIE")):
            if re.search(rf"\b{tenor}[ -]year\b", normalized):
...
def unresolved_clause_residual(clause: str) -> str:
    residual = macro_providers.normalize_quantitative_phrasing(clause)
...
def macro_model_mapping_is_safe(source: str, canonical: str) -> tuple[bool, str]:
    ...
    if not source_families.issubset(canonical_families):
        return False, (
            "concept family changed or was dropped from "
...
def resolve_prompt_series(
    ...
    *,
    ...
    prompt, normalization_notices = normalize_macro_concept_phrasing(prompt)
...
def apply_macro_chart_clarifications(
    ...
    chart_config["axes"]["left"]["title"] = title
...
def parse_chart_intent(prompt: str, series: list[dict[str, Any]]) -> dict[str, Any]:
    text = prompt.lower()
    palette = ["#0b65c2", "#d1495b", "#2a9d6f", "#f59e0b", "#6d5bd0", "#0f8b8d"]
    dual_axis_requested = bool(
        re.search(r"\b(?:axis|axes)\b", text)
        and re.search(r"\b(?:one side|other side|left|right|separate)\b", text)
    )
    ...
    if dual_axis_requested:
        recognized.append("Dual-axis request recognized: rates and yields are separated from prices and indexes.")

    specs: list[dict[str, Any]] = []
    for index, row in enumerate(series):
        name = str(row.get("name") or "").lower()
        unit = str(row.get("unit") or "").lower()
        is_rate = "percent" in unit or bool(re.search(r"\b(?:yield|rate|fed funds)\b", name))
        if dual_axis_requested:
            if yields_right or prices_left:
        ...
        specs.append(
            {
                ...
                "units": (
                    "raw"
                    if dual_axis_requested or row.get("preferredUnits") == "raw"
                    else "index"
                ),
                "frequency": "original",
                "aggregation": "average",
    ...
    if re.search(r"\b(?:bar|column)\s+chart\b", text):
        for spec in specs:
            spec["type"] = "bar"
        recognized.append("Bar chart requested.")
    elif re.search(r"\barea\s+chart\b", text):
        for spec in specs:
            spec["type"] = "area"
        recognized.append("Area chart requested.")
    elif re.search(r"\bscatter\s+(?:plot|chart)\b", text):
        for spec in specs:
            spec["type"] = "scatter"
        recognized.append("Scatter chart requested.")
    ...
    if re.search(
        ...
        for spec in specs:
            spec["units"] = "raw"
        recognized.append("Official provider index levels requested.")
    ...
    left_rows = [row for row, spec in zip(series, specs) if spec["axis"] == "left"]
    right_rows = [row for row, spec in zip(series, specs) if spec["axis"] == "right"]

    def axis_title(rows: list[dict[str, Any]]) -> str:
        units = {str(row.get("unit") or "Value") for row in rows}
        if not rows:
            return ""
        return next(iter(units)) if len(units) == 1 else " / ".join(sorted(units))
    ...
    return {
        ...
        "axes": {
            "left": {"title": axis_title(left_rows), "log": False, "min": None, "max": None},
            "right": {"title": axis_title(right_rows), "log": False, "min": None, "max": None},
        },
        "recognizedInstructions": recognized,
    }
...
def handle_fred_query(
    ...
    if not selected or remaining_missing:
        ...
        if model_status["ready"] and not protected_sources:
            ...
            model_prompt = prompt if not selected else "; ".join(source_concepts)
    ...
    chart_config = parse_chart_intent(prompt, series_results)
    ...
    return {
        ...
        "notes": [
            "Each series keeps its own explicit transformation. Dual-axis requests default to raw values; single-axis comparisons default to an index of 100.",
