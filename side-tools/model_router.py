...
def macro_intent(prompt: str) -> tuple[list[str], str]:
    parsed, model = generate_json(
        system=(
            "Map an unfamiliar macro-data phrase to one or more exact concepts allowed by the "
            ...
            "code, or commentary. The deterministic resolver, not you, selects every data series."
        ),
        user=prompt,
