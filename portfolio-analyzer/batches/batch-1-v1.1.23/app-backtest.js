...
PA.Dashboard = {
  ...
  renderHeatmapChart(containerId, heatmapValues, maxAbs) {
    ...
    const colors = heatmapValues.map(item => Number.isFinite(item.selectedReturn) ? item.selectedReturn : 0);
    const values = heatmapValues.map(item => {
      const assetScale = Number(item.quote?.netAssets ?? item.quote?.marketCap);
      return Number.isFinite(assetScale) && assetScale > 0 ? assetScale : 1;
    });
    const text = heatmapValues.map(item => item.selectedReturn != null ? PA.Fmt.pct(item.selectedReturn) : 'N/A');
    const customdata = heatmapValues.map(item => [
      item.name,
      item.group,
    ...
    const trace = {
      type: 'treemap',
      labels: heatmapValues.map(item => item.ticker),
      parents: heatmapValues.map(item => item.group),
      values,
      text,
    ...
    el.on?.('plotly_click', event => {
      const ticker = event?.points?.[0]?.label;
      if (!heatmapValues.some(item => item.ticker === ticker)) return;
  ...
  async loadMarketOverview(options={}) {
    ...
    try {
      ...
      const tickers = this.universe.map(item => item.ticker);
      const [quotes, returnHistories, correlationHistories] = await Promise.all([
        PA.API.getQuote(tickers),
        PA.API.getHistories(tickers, this.requiredHistoryRange(this.heatmapRange)),
        PA.API.getHistories(tickers, '1Y')
      ]);
      const quoteMap = {};
      (quotes || []).forEach(q => { quoteMap[q.symbol] = q; });
      ...
      const heatmapValues = this.universe.map(item => {
      ...
      const corrCommonDates = PA.Compare.intersectDates(tickers.map(ticker => correlationDataMap[ticker]?.dates || []));
      const corrReturnMap = {};
      tickers.forEach(ticker => {
      ...
      const corrHtml = PA.Compare.buildCorrelationMatrix(tickers, corrReturnMap);
      ...
      container.innerHTML = `
        <div class="market-dashboard-stack">
          <div class="market-toolbar">
            <div class="market-note">ETF market dashboard using live quotes plus trailing return windows from the local backend. Last updated ${PA.UI.escapeHtml(this.lastLoadedLabel())}.</div>
            <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
              <div class="market-timeframe-buttons">
                ${['1D','1W','1M','YTD','1Y','10Y'].map(range => `
                  <button class="range-btn ${range === this.heatmapRange ? 'active' : ''}" onclick="PA.Dashboard.setHeatmapRange('${range}')">${range}</button>
          ...
          <div>
            <div class="market-section-title">Market Heatmap (${this.heatmapRange})</div>
