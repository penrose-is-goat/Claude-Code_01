...
PA.API = {
  ...
  async getHistories(tickers, rangeKey='1Y', options={}) {
    ...
    if (unresolved.length) {
      throw this.recordError(
        new Error(`No price history returned by the local backend for ${unresolved.join(', ')}.`),
        { endpoint: 'history', symbols: unresolved, rangeKey, interval }
      );
    }
