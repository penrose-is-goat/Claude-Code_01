...
This backend serves live market data to the frontend using `yfinance`, plus FRED history for supported macro/index Strategy Lab series such as `DGS10` and `SP500`.
...
- Strategy Lab can request supported FRED series directly, which is how prompts such as `10-Yr treasury yield hits a new year to date high` now stay aligned with the underlying economic series.
