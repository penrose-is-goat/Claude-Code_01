...
PA.Dashboard = {
  ...
  heatmapModes: {
    ...
    indexes: {
      ...
      items: [
        { ticker:'SPY', name:'S&P 500', group:'US Equity', representedValue: 65000000000000 },
        { ticker:'QQQ', name:'Nasdaq 100', group:'US Equity', representedValue: 32000000000000 },
        { ticker:'DIA', name:'Dow Industrials', group:'US Equity', representedValue: 14000000000000 },
        ...
        { ticker:'RSP', name:'Equal Weight S&P 500', group:'US Equity', representedValue: 65000000000000 },
  ...
  currentModeLabel() {
    if (this.drilldown?.sourceTicker) return `${this.drilldown.sourceTicker} Holdings`;
    return this.heatmapModes[this.heatmapMode]?.label || this.heatmapModes.sectors.label;
  },

  currentModeTitle() {
    if (this.drilldown?.sourceTicker) return `${this.drilldown.sourceTicker} Holdings Heatmap`;
  ...
  async getDashboardHistories(tickers=[], range='1Y', options={}) {
    ...
    return this.setDashboardCache(key, await PA.API.getHistories(tickers, range, { allowPartial: true }));
  },
  ...
  async loadHoldingUniverse(sourceTicker='SPY') {
    const symbol = String(sourceTicker || 'SPY').toUpperCase();
    const maxHoldings = 505;
    const cached = this.holdingsCache.get(symbol);
    if (cached && Date.now() - cached.loadedAt < this.HOLDINGS_CACHE_MS) {
      this.lastUniverseNote = cached.note;
      return cached.items;
    }

    try {
      const summary = await PA.API.getQuoteSummary(symbol);
      const holdings = summary?.fundProfile?.holdings || [];
      const normalized = holdings
        .map(holding => {
          const ticker = this.normalizeHoldingSymbol(holding.symbol);
          if (!ticker || ticker === 'CASH' || ticker === 'USD') return null;
          const weight = this.normalizeHoldingWeight(holding.weight);
          return {
            ticker,
            name: holding.description || ticker,
        ...
        .filter(Boolean)
        .sort((a, b) => (Number(b.sourceWeight || 0) - Number(a.sourceWeight || 0)) || a.ticker.localeCompare(b.ticker));

      if (normalized.length) {
        const items = normalized.slice(0, maxHoldings);
        const note = items.length < normalized.length
          ? `${symbol} holdings from the backend fund profile; showing ${items.length} of ${normalized.length} holdings.`
          : `${symbol} holdings from the backend fund profile; showing all ${items.length} returned holdings.`;
        this.holdingsCache.set(symbol, { loadedAt: Date.now(), items, note });
    ...
    const note = symbol === 'SPY'
      ? `${symbol} holdings were unavailable from the backend fund profile, so this is a fallback top-holdings subset.`
      : `${symbol} holdings were unavailable from the backend fund profile. Try Refresh Now after the backend finishes loading fund data.`;
    this.holdingsCache.set(symbol, { loadedAt: Date.now(), items: fallback, note });
    this.lastUniverseNote = note;
    return fallback;
  },
  ...
  async resolveUniverse() {
    ...
    const mode = this.heatmapModes[this.heatmapMode] || this.heatmapModes.sectors;
    this.lastUniverseNote = '';
    return mode.items || [];
  },
  ...
  scheduleHeatmapPrefetch() {
    ...
    this.prefetchTimer = setTimeout(() => this.prefetchTopLevelHeatmaps(), 800);
  },
  ...
  openHeatmapTicker(ticker) {
    const symbol = String(ticker || '').toUpperCase();
    if (this.drilldown || !symbol) {
    ...
    return `
      <div class="market-return-grid">
        ${heatmapValues.map(item => `
          <div class="market-return-tile" style="background:${this.returnColor(item.selectedReturn, maxAbs)}" onclick="PA.Dashboard.openHeatmapTicker('${item.ticker}')">
            <div>
  ...
  renderHeatmapChart(containerId, heatmapValues, maxAbs) {
    ...
    const customdata = [
      ['Broad heatmap', 'All groups', 'N/A', 'N/A', 'N/A', 'Tile size uses represented market size or fund holding weight.'],
      ...
      ...leafItems.map(item => [
        ...
        Number(item.sourceWeight) > 0
          ...
          ? `Represented market: ${PA.Fmt.compact(Number(item.representedValue))}`
          : 'Equal-size tile'
      ])
    ];
    ...
    el.on?.('plotly_click', event => {
      ...
      this.openHeatmapTicker(ticker);
    });
    PA.Charts.registerPlotly(containerId, el);
  },

  async loadCorrelationPanel(tickers=[], renderKey='', options={}) {
    const panel = document.getElementById('market-correlation-panel');
    if (!panel) return;
    if (tickers.length > 30) {
      panel.innerHTML = '<div class="market-note">Correlation matrix skipped for large holding maps so the heatmap can render quickly. Use the Compare tab for deeper multi-stock correlation work.</div>';
      return;
    }
    try {
      const correlationHistories = await this.getDashboardHistories(tickers, '1Y', options);
      ...
      panel.innerHTML = corrTickers.length >= 2 && corrCommonDates.length >= 30
  ...
  async loadMarketOverview(options={}) {
    ...
    try {
      ...
      const returnHistories = await this.getDashboardHistories(tickers, this.requiredHistoryRange(this.heatmapRange), options);
      ...
      const heatmapValues = universe.map(item => {
        ...
        const dailyReturn = this.computeWindowReturn(history, '1D', null);
        return {
          ...item,
          price: this.latestHistoryPrice(history),
          group: item.group || 'Other',
          dailyReturn,
          selectedReturn: this.computeWindowReturn(history, this.heatmapRange, null)
        };
      });
          ...
          <table class="data-table market-table">
            <thead>
              <tr>
                ...
                <th>Name</th>
                <th>Group</th>
                ${hasWeights ? '<th class="right">Weight</th>' : ''}
            ...
            <tbody>
              ${heatmapValues.map(item => {
                return `
                  <tr>
                    <td style="color:var(--accent);font-weight:600;cursor:pointer" onclick="PA.Dashboard.openHeatmapTicker('${item.ticker}')">${item.ticker}</td>
                    <td>${PA.UI.escapeHtml(item.name)}</td>
                    <td>${PA.UI.escapeHtml(item.group)}</td>
                    ${hasWeights ? `<td class="right">${Number(item.sourceWeight) > 0 ? PA.Fmt.pct(Number(item.sourceWeight)) : 'N/A'}</td>` : ''}
                    <td class="right">${PA.Fmt.currency(item.price)}</td>
      ...
      container.innerHTML = `
        <div class="market-dashboard-stack">
          <div class="market-toolbar">
            <div class="market-note">${PA.UI.escapeHtml(this.currentModeLabel())} using lightweight backend price-history windows for faster heatmap rendering. Last updated ${PA.UI.escapeHtml(this.lastLoadedLabel())}.</div>
            <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
              <select class="form-select market-mode-select" onchange="PA.Dashboard.setHeatmapMode(this.value)">
                ${this.topLevelHeatmapModes().map(([key, mode]) => `
                  <option value="${key}" ${key === this.heatmapMode ? 'selected' : ''}>${PA.UI.escapeHtml(mode.label)}</option>
          ...
          <div>
            ...
            <div class="market-note" style="margin-bottom:12px">Click any ETF/index tile, such as <strong>SPY</strong> or <strong>XLF</strong>, to drill into its holdings in this same panel. Top-level tile size uses representative index or market size, not ETF AUM; holdings tile size uses fund weight when available.</div>
      ...
      const renderKey = `${this.heatmapMode}:${tickers.join(',')}`;
      this.lastCorrelationKey = renderKey;
      this.loadCorrelationPanel(tickers, renderKey, options);
