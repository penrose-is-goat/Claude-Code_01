...
PA.Dashboard = {
  ...
  heatmapModes: {
    sp500: {
      ...
      sourceNote: 'Uses Vanguard VOO official S&P 500 holdings as the public constituent source.'
    },
    sectors: {
    ...
    try {
      ...
      const normalized = holdings
        .map((holding, index) => {
          ...
          const group = (holding.sector && holding.sector !== '-') ? holding.sector : (holding.assetClass || `${symbol} Holdings`);
          const tileValue = estimatedMarketCap || sourceMarketValue || weight || 0.00001;
          return {
            ticker,
      ...
      if (normalized.length) {
        ...
        const sizeBasis = representedUniverseValue
          ? ` Tile size is estimated represented market cap: reported holding weight x ${this.formatMarketSize(representedUniverseValue)} ${options.sourceLabel || symbol} universe size.`
        ...
        const coverage = Number(payload.weightCoverage) > 0
          ...
          : '';
        const note = `${options.sourceLabel || symbol} holdings from ${provider}${asOf}; showing ${shown}${total !== shown ? ` of ${total}` : ''} holdings. ${priced} holdings have direct Yahoo price tickers for live coloring.${sizeBasis}${coverage}`;
  ...
  async loadMarketOverview(options={}) {
    ...
    try {
      ...
      const priceTickers = [...new Set(universe.map(item => this.priceTickerForItem(item)).filter(Boolean))];
          ...
          <table class="data-table market-table">
            <thead>
              <tr>
                ...
                ${hasMarketValues ? '<th class="right">Size Basis</th>' : ''}
                <th class="right">Price</th>
                <th class="right">1D</th>
                ${this.heatmapRange !== '1D' ? `<th class="right">${this.heatmapRange}</th>` : ''}
              </tr>
            </thead>
            <tbody>
              ${heatmapValues.map(item => {
                ...
                const tickerCell = openTicker
                  ? `<td style="color:var(--accent);font-weight:600;cursor:pointer" onclick="${clickHandler}">${item.ticker}</td>`
                  : `<td style="color:var(--text-secondary);font-weight:600">${item.ticker}</td>`;
                return `
                  <tr>
                    ${tickerCell}
                    ...
                    ${hasMarketValues ? `<td class="right">${PA.UI.escapeHtml(item.tileBasis || this.formatMarketSize(item.sourceMarketValue || item.tileValue || item.representedValue))}</td>` : ''}
                    ...
                    ${this.heatmapRange !== '1D' ? `<td class="right ${PA.Fmt.colorClass(item.selectedReturn)}">${item.selectedReturn != null ? PA.Fmt.pct(item.selectedReturn) : 'N/A'}</td>` : ''}
                  </tr>
                `;
              }).join('')}
            </tbody>
      ...
      container.innerHTML = `
        <div class="market-dashboard-stack">
          ...
          <div>
            ...
            <div class="market-note" style="margin-bottom:12px">Click an ETF/index tile, such as <strong>SPY</strong> or <strong>XLF</strong>, to drill into its holdings in this same panel. On constituent maps, click a stock tile to open its ticker page. Tile area is market-cap style: represented universe value x holding weight when available, then reported market value, then reported weight.</div>
          ...
          <div>
            <div class="market-section-title">${this.drilldown || this.heatmapModes[this.heatmapMode]?.sourceTicker ? 'Constituent List' : 'ETF Market List'}</div>
            ${listHtml}
          </div>
          <div>
      ...
      const renderKey = `${this.drilldown?.sourceTicker || this.heatmapMode}:${priceTickers.join(',')}`;
      this.lastCorrelationKey = renderKey;
      this.loadCorrelationPanel(priceTickers, renderKey, options);
