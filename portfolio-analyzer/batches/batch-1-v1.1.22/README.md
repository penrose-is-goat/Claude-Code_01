...
What Batch 1 v1.1.22 is meant to handle well:
...
```powershell
powershell -ExecutionPolicy Bypass -File .\start-v1.1.22.ps1
```
...
Batch 1 v1.1.22 uses a local market-data backend for:
...
7. Footer shows `Batch 1 v1.1.22`
...
- Strategy Lab now maps `S&P` prompts to `SP500` and `10-Yr treasury yield` prompts to `DGS10`, so macro event studies no longer silently run against ETF proxies like `TLT`
- Added direct FRED history support in the backend for supported Strategy Lab economic/index series, bringing webpage studies closer to the plugin’s data contract
- Strategy Lab now shows a plugin-style overlap check table so `Every signal` and `Overlap suppressed` counts/results are visible side by side
- Strategy Lab labels are clearer for FRED-backed series, including `S&P 500` and `10-year Treasury yield`, instead of exposing raw ticker codes everywhere
- The dashboard now renders a real auto-refreshing market heatmap instead of the old misleading tile list, with clear notes about free polling versus licensed streaming
- Date labels are now full and consistent across the site’s time-series charts, including drawdown views
- `Test Backend` now verifies both quote data and FRED history support so Strategy Lab setup is less confusing to validate
- Added `start-v1.1.22.*` launchers plus a `batches/batch-1-v1.1.22/` snapshot so this release runs the same way as the prior packaged versions
