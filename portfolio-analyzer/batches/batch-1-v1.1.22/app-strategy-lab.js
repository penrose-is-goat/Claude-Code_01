...
PA.StrategyLab = {
  ...
  aliasMap: [
    ...
    { symbol: 'DGS10', aliases: [/\bdgs10\b/, /\b10(?:\s*|-)?yr(?:\s+treasury)?(?:\s+yield)?\b/, /\b10(?:\s*|-)?year(?:\s+treasury)?(?:\s+yield)?\b/, /\b10y(?:\s+treasury)?(?:\s+yield)?\b/, /\b10(?:\s*|-)?yr note yield\b/, /\b10(?:\s*|-)?year note yield\b/] },
  ...
  inferSpec(prompt) {
    ...
    if (eventMode === 'price-extreme') {
      const extremeSpec = this.inferExtremeSpec(lower, windowSessions);
  ...
  inferExtremeSpec(lower='', fallbackWindow=252) {
    const explicitWindow = this.extractNumber(lower, [
    ...
    const direction = /\blow\b|\blows\b|\blower\b|\blowers\b|\blowest\b/.test(lower) ? 'low' : 'high';
    const strict = /\bnew\b|\bbreak(?:s|ing)?\b|\bexceed(?:s|ed|ing)?\b|\bfalls?\s+below\b/.test(lower);
    ...
    if (/\b52(?:\s+|-)?week\b|\bone year\b/.test(lower)) {
      return {
        direction,
        reference: '52-week',
    ...
    if (/\bquarter\b|\bthree months?\b/.test(lower)) {
      return {
        direction,
        reference: 'rolling',
  ...
  resultDefinitions(config, result) {
    return [
      {
        label: 'Event count',
        detail: `How many times the exact trigger fired in the sample. Current run found ${result.eventCount} signals.`
      },
      {
        label: 'Win rate',
      ...
      {
        label: 'Signal return',
        detail: `The compounded result of taking each signal one after another using the selected entry timing${config.suppressOverlap ? ' with overlapping events suppressed' : ''}.`
      }
    ];
  },
  ...
  async run() {
    ...
    try {
      ...
      const histories = await PA.API.getHistories([config.targetSymbol, config.contextSymbol], config.range);
      const targetHistory = PA.API.parseHistory(histories[config.targetSymbol]);
      const contextHistory = PA.API.parseHistory(histories[config.contextSymbol]);
      ...
      if (aligned.dates.length < Math.max(40, config.windowSessions + config.forwardSessions + 10)) {
        throw new Error(`Not enough overlapping history between ${this.labelSymbol(config.targetSymbol)} and ${this.labelSymbol(config.contextSymbol)} for this setup.`);
      }
  ...
  renderAuditChart(config, result) {
    ...
    if (!el) return;
    const targetLabel = this.labelSymbol(config.targetSymbol);
    const contextLabel = this.labelSymbol(config.contextSymbol);
  ...
  renderResults(config, result) {
    ...
    const targetLabel = this.labelSymbol(config.targetSymbol, { withSource: true });
    const contextSymbolLabel = this.labelSymbol(config.contextSymbol, { withSource: true });
      ...
      <div class="card">
        ...
        <div class="strategy-results-grid">
          <div class="strategy-kpi">
            <div class="strategy-kpi-label">Event Count</div>
            <div class="strategy-kpi-value">${result.eventCount}</div>
            <div class="strategy-kpi-detail">${PA.Fmt.pct(result.coverage)} of ${result.validWindows} valid ${config.forwardSessions}D windows triggered the condition.</div>
          </div>
          <div class="strategy-kpi">
            <div class="strategy-kpi-label">${PA.UI.escapeHtml(requestedStats.label)}</div>
            <div class="strategy-kpi-value">${PA.Fmt.pct(requestedStats.value)}</div>
            <div class="strategy-kpi-detail">Baseline ${PA.Fmt.pct(requestedStats.baseline)} | Edge ${PA.Fmt.pct(requestedStats.edge)}</div>
          </div>
          <div class="strategy-kpi">
            <div class="strategy-kpi-label">Average ${config.forwardSessions}D Return</div>
            <div class="strategy-kpi-value ${PA.Fmt.colorClass(result.meanReturn)}">${PA.Fmt.pct(result.meanReturn)}</div>
            <div class="strategy-kpi-detail">Median ${PA.Fmt.pct(result.medianReturn)} | Bootstrap mean CI ${PA.Fmt.pct(result.ci.low)} to ${PA.Fmt.pct(result.ci.high)}</div>
          </div>
          <div class="strategy-kpi">
            <div class="strategy-kpi-label">Long Signal Return</div>
            ...
            <div class="strategy-kpi-detail">Compounded by taking every signal long in sequence.</div>
          </div>
          <div class="strategy-kpi">
            <div class="strategy-kpi-label">Short Signal Return</div>
            <div class="strategy-kpi-value ${PA.Fmt.colorClass(result.tradeCurve.shortTotalReturn)}">${PA.Fmt.pct(result.tradeCurve.shortTotalReturn)}</div>
            <div class="strategy-kpi-detail">Compounded by taking every signal short in sequence.</div>
          </div>
          <div class="strategy-kpi">
