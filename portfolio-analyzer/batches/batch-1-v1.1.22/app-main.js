    ...
    settingsGrid.insertAdjacentHTML('afterbegin', `
      <div class="card" id="td-settings-card">
        ...
        <div style="margin-top:10px;font-size:0.8rem;color:var(--text-muted)">
          The frontend now expects a local Python backend that serves market quotes/history plus FRED macro/index series for Strategy Lab prompts such as 10-year Treasury yield and S&amp;P 500 event studies.
        </div>
        ...
        <div style="margin-top:8px;font-size:0.8rem;color:var(--text-muted)">
          Quick start: run <code>backend\\start-backend.ps1</code> or <code>start-v1.1.22.ps1</code> from PowerShell, then keep that terminal window open.
        </div>
  ...
  async testBackend() {
    try {
      ...
      if ((dgs10History?.dates || []).length < 20 || (sp500History?.dates || []).length < 20) {
        throw new Error('Backend history test returned too little FRED history for DGS10 or SP500.');
      }
      document.getElementById('status-text').textContent = 'Ready (local market-data backend running)';
      document.getElementById('status-dot').className = 'status-dot online';
      PA.UI.toast(`Backend test passed: NVDA ${PA.Fmt.currency(quote.regularMarketPrice)} plus FRED DGS10/SP500 history`, 'success');
    } catch (e) {
      document.getElementById('status-text').textContent = 'Backend offline - start backend\\start-backend.ps1';
      document.getElementById('status-dot').className = 'status-dot offline';
