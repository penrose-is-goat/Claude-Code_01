...
What Batch 1 v1.1.24 is meant to handle well:
...
```powershell
powershell -ExecutionPolicy Bypass -File .\start-v1.1.24.ps1
```
...
Batch 1 v1.1.24 uses a local market-data backend for:
...
7. Footer shows `Batch 1 v1.1.24`
...
- Added `start-v1.1.24.*` launchers plus a `batches/batch-1-v1.1.24/` snapshot so this release runs the same way as the prior packaged versions
