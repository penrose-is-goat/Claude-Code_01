...
PA.Dashboard = {
  ...
  heatmapModes: {
    ...
    sectors: {
      ...
      aggregateIndex: 'sp500',
      aggregateLabel: 'S&P 500 sector market caps',
      items: [
    ...
    indexes: {
      ...
      items: [
        ...
        { ticker:'IWM', name:'Russell 2000', group:'US Equity', representedValue: 3000000000000, sizeMethod:'proxyAdjusted' },
        { ticker:'VTI', name:'Total US Market', group:'US Equity', representedValue: 70000000000000, sizeMethod:'holdingsMarketCap' },
        { ticker:'RSP', name:'Equal Weight S&P 500', group:'US Equity', representedValue: 65000000000000, sourceIndex:'sp500' },
        { ticker:'MDY', name:'S&P MidCap 400', group:'US Equity', representedValue: 3200000000000, sizeMethod:'proxyAdjusted' },
        { ticker:'IJR', name:'S&P SmallCap 600', group:'US Equity', representedValue: 1500000000000, sizeMethod:'proxyAdjusted' }
      ]
    },
    assets: {
      label: 'Asset Classes',
      ...
      items: [
        ...
        { ticker:'IWM', name:'Russell 2000', group:'US Equity', representedValue: 3000000000000, sizeMethod:'proxyAdjusted' },
        { ticker:'VTI', name:'Total US Market', group:'US Equity', representedValue: 70000000000000, sizeMethod:'holdingsMarketCap' },
        { ticker:'EFA', name:'Developed Markets', group:'International', representedValue: 35000000000000, sizeMethod:'holdingsMarketCap' },
        { ticker:'EEM', name:'Emerging Markets', group:'International', representedValue: 30000000000000, sizeMethod:'holdingsMarketCap' },
        { ticker:'BND', name:'Aggregate US Bond Market', group:'Rates', representedValue: 53000000000000, sizeMethod:'proxyAdjusted' },
        { ticker:'TLT', name:'US Treasury Market', group:'Rates', representedValue: 37000000000000, sizeMethod:'proxyAdjusted' },
        { ticker:'LQD', name:'Investment Grade Credit', group:'Credit', representedValue: 8000000000000, sizeMethod:'proxyAdjusted' },
        { ticker:'HYG', name:'High Yield Credit', group:'Credit', representedValue: 1300000000000, sizeMethod:'proxyAdjusted' },
        { ticker:'GLD', name:'Gold', group:'Real Assets', representedValue: 17000000000000, sizeMethod:'proxyAdjusted' },
        { ticker:'UUP', name:'US Dollar Index', group:'FX', representedValue: 6000000000000, sizeMethod:'proxyAdjusted' }
      ]
    }
  },
  ...
  getAssetSizeCache(key, options={}) {
    if (options.force) return null;
    const cached = this.assetSizeCache.get(key);
    if (!cached) return null;
    if (Date.now() - cached.loadedAt > this.HOLDINGS_CACHE_MS) {
      this.assetSizeCache.delete(key);
      return null;
    }
    return cached.value;
  },

  setAssetSizeCache(key, value) {
    this.assetSizeCache.set(key, { loadedAt: Date.now(), value });
    return value;
  },
  ...
  quoteMap(quotes=[]) {
    return new Map((quotes || []).map(quote => [this.normalizeHoldingSymbol(quote.symbol || quote.ticker), quote]));
  },

  quotePrice(quote={}) {
    return this.validPositiveNumber(quote.regularMarketPrice)
      || this.validPositiveNumber(quote.price)
      || this.validPositiveNumber(quote.navPrice)
      || null;
  },

  quoteDailyReturn(quote={}) {
    const value = Number(quote.regularMarketChangePercent ?? quote.changePercent ?? quote.percentChange);
    return Number.isFinite(value) ? value / 100 : null;
  },

  proxyAdjustedValue(item={}, dailyReturn=null) {
    const baseline = this.validPositiveNumber(item.representedValue);
    if (!baseline) return null;
    const change = Number(dailyReturn);
    return Number.isFinite(change) ? baseline * (1 + change) : baseline;
  },

  sectorProxyForName(sector='') {
    ...
    const map = {
      ...
      'healthcare': { ticker:'XLV', name:'Health Care' },
      'consumer discretionary': { ticker:'XLY', name:'Consumer Discretionary' },
      'communication services': { ticker:'XLC', name:'Communication Services' },
      'communications': { ticker:'XLC', name:'Communication Services' },
      'industrials': { ticker:'XLI', name:'Industrials' },
      'consumer staples': { ticker:'XLP', name:'Consumer Staples' },
    ...
    return map[key] || { ticker: this.normalizeHoldingSymbol(sector).slice(0, 8) || 'OTHER', name: sector || 'Other' };
  },

  async loadIndexMarketCapSnapshot(indexKey='sp500', options={}) {
    const key = `index-size:${String(indexKey || '').toLowerCase()}`;
    const cached = this.getAssetSizeCache(key, options);
    if (cached) return cached;
    const payload = await PA.API.getIndexConstituents(indexKey, { limit: 650 });
    const constituents = payload?.constituents || [];
    const summedMarketCap = constituents.reduce((sum, item) => {
      const value = this.validPositiveNumber(item.marketCap);
      return value ? sum + value : sum;
    }, 0);
    const value = this.validPositiveNumber(payload?.totalMarketCap) || this.validPositiveNumber(summedMarketCap);
    return this.setAssetSizeCache(key, {
      value,
      provider: payload?.provider || 'backend index constituents',
      asOf: payload?.asOf || null,
      count: payload?.count || constituents.length,
      displayCount: payload?.displayCount || constituents.length
    });
  },

  async loadHoldingsMarketCapSnapshot(symbol='', options={}) {
    const ticker = this.normalizeHoldingSymbol(symbol);
    const key = `holdings-size:${ticker}`;
    const cached = this.getAssetSizeCache(key, options);
    if (cached) return cached;
    const payload = await PA.API.getEtfHoldings(ticker);
    const holdings = payload?.holdings || [];
    let counted = 0;
    const value = holdings.reduce((sum, holding) => {
      const marketCap = this.validPositiveNumber(holding.marketCap);
      if (!marketCap) return sum;
      counted += 1;
      return sum + marketCap;
    }, 0);
    return this.setAssetSizeCache(key, {
      value: this.validPositiveNumber(value),
      provider: payload?.provider || 'backend holdings',
      asOf: payload?.asOf || null,
      count: payload?.count || holdings.length,
      counted,
      weightCoverage: payload?.weightCoverage || null
    });
  },

  async loadSectorAggregateUniverse(mode={}, options={}) {
    const key = `sector-aggregate:${mode.aggregateIndex || 'sp500'}`;
    const cached = this.getAssetSizeCache(key, options);
    if (cached) {
      this.lastUniverseNote = cached.note;
      return cached.items;
    }
    const payload = await PA.API.getIndexConstituents(mode.aggregateIndex || 'sp500', { limit: 650 });
    const constituents = payload?.constituents || [];
    const groups = new Map();
    constituents.forEach(constituent => {
      const sector = constituent.sector || constituent.industry || 'Other';
      const marketCap = this.validPositiveNumber(constituent.marketCap);
      if (!marketCap) return;
      const dailyReturn = Number.isFinite(Number(constituent.dailyReturn)) ? Number(constituent.dailyReturn) : null;
      const current = groups.get(sector) || { sector, marketCap: 0, weightedReturn: 0, returnWeight: 0, count: 0 };
      current.marketCap += marketCap;
      current.count += 1;
      if (Number.isFinite(dailyReturn)) {
        current.weightedReturn += dailyReturn * marketCap;
        current.returnWeight += marketCap;
      }
      groups.set(sector, current);
    });
    const sectorTickers = [...groups.values()].map(group => this.sectorProxyForName(group.sector).ticker).filter(Boolean);
    let quotes = [];
    try {
      quotes = await this.getDashboardQuotes(sectorTickers, options);
    } catch (error) {
      console.warn('Sector proxy quote lookup failed:', error);
    }
    const quotesByTicker = this.quoteMap(quotes);
    const items = [...groups.values()].map(group => {
      const proxy = this.sectorProxyForName(group.sector);
      const quote = quotesByTicker.get(this.normalizeHoldingSymbol(proxy.ticker)) || {};
      const dailyReturn = group.returnWeight > 0 ? group.weightedReturn / group.returnWeight : this.quoteDailyReturn(quote);
      return {
        ticker: proxy.ticker,
        name: proxy.name,
        group: 'S&P 500 Sectors',
        sourceTicker: proxy.ticker,
        sourceMarketValue: group.marketCap,
        sourceDailyReturn: dailyReturn,
        sourcePrice: this.quotePrice(quote),
        sourceRepresentedValue: group.marketCap,
        representedValue: group.marketCap,
        tileValue: group.marketCap,
        tileBasis: `Current S&P 500 sector market cap from ${group.count} constituents: ${this.formatMarketSize(group.marketCap)}`,
        holdingsLeaf: false,
        priceTicker: proxy.ticker,
        openTicker: proxy.ticker,
        heatmapId: `sector-${proxy.ticker}`
      };
    }).sort((a, b) => Number(b.tileValue || 0) - Number(a.tileValue || 0));
    const asOf = payload?.asOf ? ` as of ${payload.asOf}` : '';
    const note = `${mode.aggregateLabel || 'S&P 500 sectors'} are dynamically sized from current constituent market caps${asOf}; sector color uses constituent-weighted 1D return when available and ETF proxy history for longer windows.`;
    this.lastUniverseNote = note;
    return this.setAssetSizeCache(key, { items, note }).items;
  },

  async loadStaticModeUniverse(mode={}, options={}) {
    const baseItems = (mode.items || []).map(item => ({ ...item }));
    const tickers = [...new Set(baseItems.map(item => this.normalizeHoldingSymbol(item.ticker)).filter(Boolean))];
    let quotes = [];
    try {
      quotes = await this.getDashboardQuotes(tickers, options);
    } catch (error) {
      console.warn('Dashboard quote sizing lookup failed:', error);
    }
    const quotesByTicker = this.quoteMap(quotes);
    const indexKeys = [...new Set(baseItems.map(item => item.sourceIndex).filter(Boolean))];
    const holdingsSymbols = [...new Set(baseItems
      .filter(item => item.sizeMethod === 'holdingsMarketCap')
      .map(item => this.normalizeHoldingSymbol(item.ticker))
      .filter(Boolean))];
    const indexSnapshots = new Map();
    await Promise.all(indexKeys.map(async indexKey => {
      try {
        indexSnapshots.set(indexKey, await this.loadIndexMarketCapSnapshot(indexKey, options));
      } catch (error) {
        console.warn(`Index size lookup failed for ${indexKey}:`, error);
      }
    }));
    const holdingsSnapshots = new Map();
    await Promise.all(holdingsSymbols.map(async symbol => {
      try {
        holdingsSnapshots.set(symbol, await this.loadHoldingsMarketCapSnapshot(symbol, options));
      } catch (error) {
        console.warn(`Holdings size lookup failed for ${symbol}:`, error);
      }
    }));

    const items = baseItems.map(item => {
      const ticker = this.normalizeHoldingSymbol(item.ticker);
      const quote = quotesByTicker.get(ticker) || {};
      const dailyReturn = this.quoteDailyReturn(quote);
      const indexSnapshot = item.sourceIndex ? indexSnapshots.get(item.sourceIndex) : null;
      const holdingsSnapshot = item.sizeMethod === 'holdingsMarketCap' ? holdingsSnapshots.get(ticker) : null;
      let representedValue = this.validPositiveNumber(indexSnapshot?.value);
      let tileBasis = representedValue
        ? `Current index constituent market cap: ${this.formatMarketSize(representedValue)}${indexSnapshot?.asOf ? ` as of ${indexSnapshot.asOf}` : ''}`
        : '';
      if (!representedValue && holdingsSnapshot?.value) {
        representedValue = holdingsSnapshot.value;
        tileBasis = `Current holdings market-cap sum: ${this.formatMarketSize(representedValue)} across ${holdingsSnapshot.counted || 0} priced holdings${holdingsSnapshot.asOf ? ` as of ${holdingsSnapshot.asOf}` : ''}`;
      }
      if (!representedValue && item.sizeMethod === 'proxyAdjusted') {
        representedValue = this.proxyAdjustedValue(item, dailyReturn);
        tileBasis = `Proxy-adjusted represented market: ${this.formatMarketSize(representedValue)} from ${ticker} 1D move; baseline estimate ${this.formatMarketSize(item.representedValue)}`;
      }
      if (!representedValue) {
        representedValue = this.validPositiveNumber(item.representedValue) || this.validPositiveNumber(quote.marketCap) || this.validPositiveNumber(quote.netAssets) || 1;
        tileBasis = `Fallback represented market baseline: ${this.formatMarketSize(representedValue)}`;
      }
      return {
        ...item,
        ticker,
        name: item.name || ticker,
        sourceTicker: item.sourceTicker || ticker,
        sourceMarketValue: representedValue,
        sourceDailyReturn: dailyReturn,
        sourcePrice: this.quotePrice(quote),
        sourceRepresentedValue: representedValue,
        representedValue,
        tileValue: representedValue,
        tileBasis,
        priceTicker: ticker,
        openTicker: ticker,
        holdingsLeaf: false,
        heatmapId: `mode-${mode.label || 'market'}-${ticker}`
      };
    }).sort((a, b) => Number(b.tileValue || 0) - Number(a.tileValue || 0));

    const dynamicCount = items.filter(item => !String(item.tileBasis || '').startsWith('Fallback')).length;
    this.lastUniverseNote = `${mode.label || 'Dashboard'} tile sizes are dynamic where free public data supports it: index constituent market caps for index proxies, holdings market-cap sums for broad equity ETFs, and proxy-adjusted represented-market baselines for non-equity markets such as bonds, gold, credit, and FX. ${dynamicCount} of ${items.length} tiles used a dynamic sizing path on this load.`;
    return items;
  },
  ...
  async resolveUniverse(options={}) {
    ...
    this.lastUniverseNote = '';
    if (mode.aggregateIndex) {
      return this.loadSectorAggregateUniverse(mode, options);
    }
    if (mode.sourceIndex) {
    ...
    return this.loadStaticModeUniverse(mode, options);
  },
  ...
  correlationReferenceGroups() {
    return [
      ...
      {
        key: 'asset-classes',
        label: 'Total Market / Asset Classes',
  ...
  fixedCorrelationShell(groups=[]) {
    return `
      <div class="market-note" style="margin-bottom:10px">These are fixed 1Y daily-return reference matrices. They stay the same when you click into a heatmap or drill into ETF holdings.</div>
      <div class="correlation-matrix-stack">
        ${groups.map(group => `
          <section class="correlation-matrix-panel">
            <div class="market-section-title">${PA.UI.escapeHtml(group.label)}</div>
            <div id="correlation-matrix-${PA.UI.escapeHtml(group.key)}" class="correlation-matrix-body">
              <div class="market-note">Preparing ${PA.UI.escapeHtml(group.label)} matrix...</div>
            </div>
          </section>
        `).join('')}
      </div>
  ...
  async renderCorrelationGroup(group={}, options={}) {
    const target = document.getElementById(`correlation-matrix-${group.key}`);
    if (!target) return;
    const tickers = [...new Set((group.items || []).map(item => item.ticker).filter(Boolean))];
    if (tickers.length < 2) {
      target.innerHTML = '<div class="market-note">Correlation matrix needs at least two tickers.</div>';
      return;
    }
    try {
      const correlationHistories = await this.getDashboardHistories(tickers, '1Y', {
        ...options,
        maxFallbacks: Math.min(Math.max(tickers.length, 2), 12)
      });
      ...
      target.innerHTML = corrTickers.length >= 2 && corrCommonDates.length >= 30
        ? PA.Compare.buildCorrelationMatrix(corrTickers, corrReturnMap)
        : '<div class="market-note">Correlation matrix needs at least two tickers with overlapping 1Y history.</div>';
    } catch (error) {
      target.innerHTML = `<div class="market-note">Correlation matrix unavailable: ${PA.UI.escapeHtml(PA.UI.errorText(error))}</div>`;
    }
  },

  async loadCorrelationPanel(_tickers=[], _renderKey='', options={}) {
    const panel = document.getElementById('market-correlation-panel');
    if (!panel) return;
    const groups = this.correlationReferenceGroups();
    panel.innerHTML = this.fixedCorrelationShell(groups);
    groups.forEach(group => this.renderCorrelationGroup(group, options));
  },
      ...
      container.innerHTML = `
        <div class="market-dashboard-stack">
          ...
          <div>
            ...
            <div id="market-correlation-panel">${this.fixedCorrelationShell(this.correlationReferenceGroups())}</div>
          </div>
        </div>
      `;
      this.renderHeatmapChart('market-heatmap-chart', heatmapValues, maxAbs);
      this.loadCorrelationPanel([], 'fixed-reference-correlations', { ...options, maxFallbacks: 12 });
