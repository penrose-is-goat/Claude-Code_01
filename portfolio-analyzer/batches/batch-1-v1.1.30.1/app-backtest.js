...
PA.Dashboard = {
    ...
    try {
      ...
      const normalized = holdings
        .map((holding, index) => {
          ...
          const group = holding.sector || holding.assetClass || `${symbol} Holdings`;
          return {
            ticker,
      ...
      if (normalized.length) {
        ...
        const coverage = Number(payload.weightCoverage) > 0
          ...
          : '';
        const note = `${symbol} holdings from ${provider}${asOf}; showing ${shown}${total !== shown ? ` of ${total}` : ''} holdings. ${priced} holdings have direct Yahoo price tickers for live coloring.${coverage}`;
    ...
    return `
      <div class="market-return-grid">
        ${heatmapValues.map(item => `
          <div class="market-return-tile" style="background:${this.returnColor(item.selectedReturn, maxAbs)}" onclick="PA.Dashboard.openHeatmapTicker('${this.openTickerForItem(item)}')">
            <div>
  ...
  renderHeatmapChart(containerId, heatmapValues, maxAbs) {
    ...
    const leafIds = leafItems.map((item, index) => {
      ...
      this.heatmapClickTargets.set(id, this.openTickerForItem(item));
      return id;
    });
    ...
    el.on?.('plotly_click', event => {
      ...
      const ticker = this.heatmapClickTargets.get(String(pointId)) || '';
            ...
            <tbody>
              ${heatmapValues.map(item => {
                ...
                const tickerCell = openTicker
                  ? `<td style="color:var(--accent);font-weight:600;cursor:pointer" onclick="PA.Dashboard.openHeatmapTicker('${openTicker}')">${item.ticker}</td>`
