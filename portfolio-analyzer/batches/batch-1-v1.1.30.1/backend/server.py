...
def frame_to_history(symbol, frame, range_key, interval="1d", provider_source="yfinance-bulk"):
    if frame is None or getattr(frame, "empty", True):
...
def normalize_bulk_yfinance_histories(symbols, range_key, interval="1d"):
    ...
    period = period_for_request(normalized_range, normalized_interval)
    provider_symbols = {symbol: symbol.replace("-", ".") for symbol in clean_symbols}
    ...
    for symbol in clean_symbols:
        ...
        if frame is None:
            continue
        try:
            histories[symbol] = frame_to_history(symbol, frame, normalized_range, normalized_interval)
        except Exception:
            continue
    return histories
...
def normalize_holding_weight(value):
    if is_finite_number(value):
        numeric = float(value)
    else:
...
def holding_from_row(row, headers, source_symbol, row_number):
    ...
    weight = normalize_holding_weight(holding_row_cell(row, weight_index))
...
def parse_spdr_holdings(symbol):
    ...
    if normalized_symbol not in SPDR_HOLDINGS_SYMBOLS:
        raise ValueError(f"{normalized_symbol} is not configured as a State Street SPDR holdings file.")
...
def parse_ishares_holdings(symbol):
    ...
    if not config:
        raise ValueError(f"{normalized_symbol} is not configured as an iShares holdings file.")
    url = (
...
def get_etf_holdings(symbol):
    ...
    if normalized_symbol in SPDR_HOLDINGS_SYMBOLS:
        providers.append(("State Street", parse_spdr_holdings))
    if normalized_symbol in ISHARES_HOLDINGS:
        providers.append(("iShares", parse_ishares_holdings))
    providers.extend([
        ("StockAnalysis", parse_stockanalysis_holdings),
        ("yfinance", holdings_from_yfinance_summary),
    ])
    ...
    def do_GET(self):
        try:
            ...
            if path == "/api/history":
                ...
                histories = {}
                errors = {}
                bulk_symbols = [symbol for symbol in symbols if not is_backend_canonical_history_symbol(symbol)]
                if len(bulk_symbols) >= 2:
                    try:
                        histories.update(normalize_bulk_yfinance_histories(bulk_symbols, range_key, interval))
                    except Exception as exc:
