...
PA.Config = {
  ...
  CHART_INTERVALS: {
    '15m': { label:'15 Min', backend:'15m', ranges:['1D','5D','1M'] },
    '1h':  { label:'1 Hour', backend:'1h', ranges:['1D','5D','1M','3M'] },
    '1d':  { label:'1 Day', backend:'1d', ranges:['1D','5D','1M','3M','6M','YTD','1Y','3Y','5Y','10Y','MAX'] },
    '1wk': { label:'1 Week', backend:'1wk', ranges:['1M','3M','6M','YTD','1Y','3Y','5Y','10Y','MAX'] },
    '1mo': { label:'1 Month', backend:'1mo', ranges:['6M','YTD','1Y','3Y','5Y','10Y','MAX'] }
  },
  CHART_COMPARE_INDEXES: {
    NONE: 'No Compare',
    SPY: 'S&P 500 (SPY)',
    QQQ: 'Nasdaq 100 (QQQ)',
    DIA: 'Dow Jones (DIA)'
...
PA.DB = {
  ...
  runSchema() {
    ...
    this.ensureColumn('portfolios', 'is_system', 'INTEGER DEFAULT 0');
  },
...
PA.API = {
  ...
  normalizeBackendError(error, requestUrl='') {
    ...
    return `Cannot reach the local market-data backend at ${target}. Start backend\\start-backend.ps1 and keep it running, then click Test Backend in Settings.`;
  },
  ...
  getDiagnostics() {
    return {
      provider: 'Local market-data backend',
  ...
  requireBackendUrl() {
    ...
    if (!url) {
      throw new Error('Missing backend URL. Start the local market-data backend and save its URL in Settings.');
    }
    return url;
  },
  ...
  async fetchBackendJson(endpoint, params={}) {
    ...
    const task = (async () => {
      ...
      try {
        ...
        this.recordRequest('Local market-data backend', endpoint, normalizedParams, requestUrl);
        ...
        throw this.recordError(normalized, { provider: 'Local market-data backend', endpoint, params: normalizedParams, requestUrl });
      }
    })();
  ...
  async getQuote(tickers, options={}) {
    ...
    if (unresolved.length && !options?.allowPartial) {
      const detail = unresolved
        .map(symbol => errors?.[symbol] ? `${symbol}: ${errors[symbol]}` : symbol)
        .join(' | ');
      throw this.recordError(
        new Error(`No quote returned by the local backend for ${detail}.`),
        { endpoint: 'quote', symbols: unresolved, errors }
      );
    }
  ...
  async getQuoteSummary(ticker) {
    ...
    if (!data?.summary) {
      throw this.recordError(
        new Error(`No summary returned by the local backend for ${symbol}.`),
  ...
  async getEtfHoldings(ticker, options={}) {
    const symbol = String(ticker).toUpperCase().trim();
    const params = { symbol };
    if (options?.limit) params.limit = options.limit;
    const data = await this.fetchBackendJson('holdings', params);
    if (!data?.holdings?.holdings?.length) {
      throw this.recordError(
        new Error(`No ETF holdings returned by the local backend for ${symbol}.`),
        { endpoint: 'holdings', symbol }
      );
    }
    return data.holdings;
  },

  async getIndexConstituents(indexKey, options={}) {
    const index = String(indexKey || '').toLowerCase().trim();
    const params = { index };
    if (options?.limit) params.limit = options.limit;
    const data = await this.fetchBackendJson('index-constituents', params);
    if (!data?.index?.constituents?.length) {
      throw this.recordError(
        new Error(`No index constituents returned by the local backend for ${index}.`),
        { endpoint: 'index-constituents', index }
      );
    }
    return data.index;
  },

  async getHistory(ticker, rangeKey='1Y', options={}) {
    const symbol = String(ticker).toUpperCase().trim();
    const result = await this.getHistories([symbol], rangeKey, options);
    return result[symbol] || null;
  },

  async getHistories(tickers, rangeKey='1Y', options={}) {
    const symbols = [...new Set((tickers || []).map(t => String(t).toUpperCase().trim()).filter(Boolean))];
    ...
    if (Number.isFinite(Number(options?.maxFallbacks))) {
      params.maxFallbacks = Math.max(0, Math.floor(Number(options.maxFallbacks)));
    }
    const data = await this.fetchBackendJson('history', params);
    ...
    if (unresolved.length && !options?.allowPartial) {
      const detail = unresolved
        ...
        .join(' | ');
      throw this.recordError(
        new Error(`No price history returned by the local backend for ${detail}.`),
        { endpoint: 'history', symbols: unresolved, rangeKey, interval, errors }
      );
    }
  ...
  parseHistory(result) {
    if (!result) return { dates:[], prices:[], adjustedPrices:[], volumes:[], interval:'1d' };
    return {
      dates: result.dates || [],
      ...
      rangeKey: result.rangeKey || '1Y',
      ticker: result.ticker || '',
      providerSource: result.providerSource || '',
      providerSeriesId: result.providerSeriesId || '',
      providerSymbol: result.providerSymbol || '',
      providerTransform: result.providerTransform || '',
      providerFallbackLabel: result.providerFallbackLabel || '',
      providerAttemptChain: Array.isArray(result.providerAttemptChain) ? result.providerAttemptChain : []
    };
  },
...
PA.Compute = {
  ...
  movingAverage(values, period) {
    ...
    const p = Math.max(1, parseInt(period, 10) || 0);
    if (!p) return result;
    let sum = 0;
  ...
  exponentialMovingAverage(values, period) {
    const result = new Array(values.length).fill(null);
    const p = Math.max(1, parseInt(period, 10) || 0);
    if (!p || !values.length) return result;
  ...
  macd(values, fast=12, slow=26, signal=9) {
    const fastEma = this.exponentialMovingAverage(values, fast);
    const slowEma = this.exponentialMovingAverage(values, slow);
    ...
    const histogram = macdLine.map((value, i) => (
      Number.isFinite(value) && Number.isFinite(signalLine[i]) ? value - signalLine[i] : null
    ));
    return { macdLine, signalLine, histogram };
  },
  normalizePerformance(values) {
    const clean = (values || []).map(v => Number(v));
    const first = clean.find(v => Number.isFinite(v) && v !== 0);
    if (!Number.isFinite(first)) return clean.map(() => null);
    return clean.map(v => Number.isFinite(v) ? (v / first) - 1 : null);
  }
};
