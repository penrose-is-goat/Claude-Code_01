...
PA.StrategyLab = {
  examples: [
    ...
    {
      label: 'Fed 25 bps hike check',
      prompt: 'Test how often the S&P is down one week after the Federal Reserve hikes by 25 basis points or more.'
    },
    {
  ...
  aliasMap: [
    ...
    { symbol: 'DFF', aliases: [/\bdff\b/, /\bfederal funds(?: rate)?\b/, /\bfed funds(?: rate)?\b/, /\bfed(?:eral)? funds rate\b/] },
    { symbol: 'DFEDTARU', aliases: [/\bdfedtaru\b/, /\bfederal funds target(?: rate)?\b/, /\bfed funds target(?: rate)?\b/, /\bfomc target(?: rate)?\b/, /\btarget federal funds rate\b/] },
  ...
  seriesCatalog: {
    ...
    DFEDTARU: {
      label: 'Federal funds target upper bound',
      canonicalSymbol: 'DFEDTARU',
      preferCanonicalSignal: true,
      signalCandidates: []
    },
    DFF: {
  ...
  isFredSymbol(symbol='') {
    return ['SP500', 'DGS10', 'DFEDTARU', 'FEDFUNDS', 'DFF'].includes(String(symbol || '').trim().toUpperCase());
  },
  ...
  labelSymbol(symbol='', options={}) {
    ...
    const labels = {
      ...
      'DFEDTARU': withSource ? 'Federal funds target upper bound (FRED)' : 'Federal funds target upper bound',
      'FEDFUNDS': withSource ? 'Effective federal funds rate (FRED monthly)' : 'Effective federal funds rate',
      'DFF': withSource ? 'Effective federal funds rate (FRED daily)' : 'Effective federal funds rate',
      'IEF': 'IEF',
      'TLT': 'TLT',
      'GLD': 'GLD',
      'UUP': 'UUP',
  ...
  providerSourceLabel(source='') {
    ...
    const labels = {
      fred: 'FRED',
      'fed-policy-table': 'Federal Reserve target-rate table',
  ...
  formatLevelValue(value, symbol='') {
    ...
    return ['DGS10', 'DFEDTARU', 'FEDFUNDS', 'DFF'].includes(String(symbol || '').trim().toUpperCase())
  ...
  inferSpec(prompt) {
    ...
    } else if (eventMode === 'level-change') {
      const changeSpec = this.inferChangeSpec(lower);
      triggerSymbol = this.inferTriggerSymbol(prompt, symbols, targetSymbol, contextSymbol);
      contextSymbol = triggerSymbol;
      relation = changeSpec.direction === 'cut' ? 'opposite-direction' : 'same-direction';
      windowSessions = changeSpec.lookbackSessions;
      threshold = changeSpec.thresholdBps;
    }
    ...
    if (eventMode === 'level-change') {
      notes.push('Rate-change mode fires when the trigger series changes by at least the requested number of basis points.');
      if (!/\b\d+(?:\.\d+)?\s*(?:basis\s+points?|bps?|bp)\b/.test(lower)) {
        notes.push('No basis-point threshold was specified, so Strategy Lab defaults the rate-change trigger to 25 bps.');
      }
      if (/\bcme\b|\bfedwatch\b|\bfed fund futures\b|\bfed funds futures\b|\bprobab/.test(lower)) {
        notes.push('CME FedWatch probability-history comparisons require a licensed/supplied FedWatch dataset. This website run covers the actual fed-funds-rate hike leg only.');
      }
    }
    ...
    return {
      ...
      extremeStrict,
      changeDirection: eventMode === 'level-change' ? (relation === 'opposite-direction' ? 'cut' : 'hike') : null,
      changeThresholdBps: eventMode === 'level-change' ? threshold : null,
      changeLookbackSessions: eventMode === 'level-change' ? windowSessions : null,
  ...
  detectSymbols(prompt) {
    ...
    const explicit = [...new Set((prompt.match(/[\$^]?[A-Z]{1,5}\b/g) || []))]
      ...
      .filter(token => {
        ...
        if (!clean || ['A', 'AN', 'AND', 'OR', 'THE', 'IN', 'UP', 'ON'].includes(clean)) return false;
        // Avoid treating the S and P fragments inside "S&P" as standalone tickers.
        if (/^[A-Z]$/.test(clean) && !token.startsWith('$') && !token.startsWith('^')) return false;
        return true;
      })
  ...
  inferContextSymbol(prompt, symbols=[], targetSymbol='SPY', eventMode='directional-streak') {
    ...
    if (eventMode === 'level-change') {
      return this.inferFedPolicySymbol(prompt) || afterSymbols[0] || symbols.find(symbol => symbol !== targetSymbol) || 'DFEDTARU';
    }
    if (eventMode === 'price-extreme') {
  ...
  inferTriggerSymbol(prompt, symbols=[], targetSymbol='SPY', contextSymbol='^VIX') {
    ...
    const afterPart = lower.includes('after') ? lower.split(/\bafter\b/).slice(1).join(' after ') : lower;
    const afterSymbols = this.matchAliases(afterPart);
    const fedPolicySymbol = this.inferFedPolicySymbol(prompt);
    if (fedPolicySymbol) return fedPolicySymbol;
  ...
  inferEventMode(lower='') {
    if (this.isFedPolicyChangePrompt(lower)) {
      return 'level-change';
    }
  ...
  isFedPolicyChangePrompt(lower='') {
    const source = String(lower || '').toLowerCase();
    const hasFedActor = /\b(?:dff|dfedtaru|fedfunds|federal reserve|the fed|fed|fomc|federal open market committee|federal funds|fed funds|fed(?:eral)? funds rate|target federal funds rate|policy rate)\b/.test(source);
    const hasRateMove = /\b(?:hike|hikes|hiked|increase|increases|increased|raise|raises|raised|cut|cuts|cutting|decrease|decreases|decreased|lower|lowers|lowered|basis points?|bps?|bp)\b/.test(source);
    return hasFedActor && hasRateMove;
  },

  inferFedPolicySymbol(prompt='') {
    const lower = String(prompt || '').toLowerCase();
    if (!this.isFedPolicyChangePrompt(lower)) return null;
    if (/\bdff\b|\beffective federal funds\b/.test(lower)) return 'DFF';
    // "The Fed/Federal Reserve hikes" is a policy-target event, not a traded ticker.
    return 'DFEDTARU';
  },

  inferChangeSpec(lower='') {
    const primaryClause = String(lower || '').split(/\b(?:and then compare|compare it with|compare with|then compare)\b/)[0] || lower;
    const thresholdMatch = primaryClause.match(/\b(\d+(?:\.\d+)?)\s*(?:basis\s+points?|bps?|bp)\b/);
    const thresholdBps = thresholdMatch ? Number(thresholdMatch[1]) : 25;
    const direction = /\b(?:cut|cuts|cutting|decrease|decreases|decreased|lower|lowers|lowered|drop|drops|dropped)\b/.test(primaryClause)
      ? 'cut'
      : 'hike';
    const lookbackSessions = /\bprior week\b|\bover the prior week\b|\bweek over week\b|\bw\/w\b/.test(primaryClause) ? 5 : 1;
    return {
      direction,
      thresholdBps: Number.isFinite(thresholdBps) && thresholdBps > 0 ? thresholdBps : 25,
      lookbackSessions
    };
  },
  ...
  getFormConfig() {
    ...
    const defaultContext = ['price-extreme', 'level-change'].includes(eventMode)
    ...
    const threshold = eventMode === 'price-extreme'
      ? 1
      : (eventMode === 'level-change'
        ? Math.max(1, parseFloat(document.getElementById('sl-threshold')?.value) || 25)
    ...
    if (eventMode === 'level-change') {
      notes.push('Rate-change mode uses the trigger symbol level change in basis points. Relation acts as Hike or Cut.');
    }
    ...
    const config = {
      ...
      changeDirection: eventMode === 'level-change'
        ? (relation === 'opposite-direction' ? 'cut' : 'hike')
        : (lastSpec.changeDirection || null),
      changeThresholdBps: eventMode === 'level-change' ? threshold : (lastSpec.changeThresholdBps || null),
      changeLookbackSessions: eventMode === 'level-change' ? windowSessions : (lastSpec.changeLookbackSessions || null),
      notes
    };
  ...
  updateFormMode(spec={}) {
    ...
    const isLevelChange = spec.eventMode === 'level-change';
    ...
    if (contextLabel) contextLabel.textContent = (isExtreme || isLevelChange) ? 'Trigger Symbol' : 'Context Symbol';
    if (relationLabel) relationLabel.textContent = isExtreme ? 'Extreme Side' : (isLevelChange ? 'Rate Move' : 'Relation');
    if (windowLabel) {
      windowLabel.textContent = isLevelChange
        ? 'Change Lookback'
        : isExtreme
        ? (spec.extremeReference === 'ytd' ? 'Reference Window (calendar)' : 'Reference Window')
        : 'Lookback Window';
    }
    if (thresholdLabel) thresholdLabel.textContent = isExtreme ? 'Trigger Threshold' : (isLevelChange ? 'Min Change (bps)' : 'Min Matching Days');

    if (relationSelect?.options?.length >= 2) {
      relationSelect.options[0].text = isExtreme ? 'High' : (isLevelChange ? 'Hike / Increase' : 'Same Direction');
      relationSelect.options[1].text = isExtreme ? 'Low' : (isLevelChange ? 'Cut / Decrease' : 'Opposite Direction');
    }
    if (windowInput) {
    ...
    if (hint) {
      hint.textContent = isLevelChange
        ? `Rate-change mode: ${this.describeTrigger(spec)}. Run uses the latest prompt automatically and treats the threshold as basis points.`
        : isExtreme
  ...
  describeTrigger(spec={}) {
    if (spec.eventMode === 'level-change') {
      const symbol = this.labelSymbol(spec.triggerSymbol || spec.contextSymbol || 'DFEDTARU');
      const side = spec.changeDirection === 'cut' || spec.relation === 'opposite-direction' ? 'falls' : 'rises';
      const bps = Number(spec.changeThresholdBps || spec.threshold || 25);
      const lookback = Number(spec.changeLookbackSessions || spec.windowSessions || 1);
      return `${symbol} ${side} by at least ${bps} bps${lookback > 1 ? ` over ${lookback} trading sessions` : ''}`;
    }
  ...
  requestedOutcomeStats(result, config) {
    ...
    if (config.outcomeDirection === 'up') {
      return {
        label: 'Win Rate',
        value: trade.winRate,
        baseline: trade.baselineWinRate,
        edge: trade.winRate - trade.baselineWinRate,
        formula: trade.winFormula
      };
    }
  ...
  hypothesisTradeStats(result={}, config={}) {
    ...
    const baselineWinRate = spec.side === 'short'
      ? result.baselineNegativeRate
      : result.baselinePositiveRate;
    const winFormula = spec.side === 'short'
      ? 'P(short return > 0) = P(forward target return < 0)'
      : 'P(long return > 0) = P(forward target return > 0)';
    return {
      ...spec,
      ...
      baselineWinRate: Number.isFinite(baselineWinRate) ? baselineWinRate : 0,
      winFormula
    };
  },
  ...
  resultDefinitions(config, result) {
    ...
    return [
      ...
      {
        label: 'Win rate',
        detail: `Whether the prompt-side trade made money. For bearish prompts this is short target wins; for bullish prompts this is long target wins.`
      },
      {
      ...
      {
        label: 'Long / Short target check',
        detail: `A side-by-side sanity check showing what happened if every signal was traded long versus short, compounded across signals and not annualized.`
      },
      {
        label: 'Credibility read',
        detail: `A quick warning label based on sample size and mean-return strength. It is a research-quality check, not proof of a tradable edge.`
      }
    ];
  },
  ...
  buildAuditWindow(auditSeries={}, events=[], maxPoints=null) {
    const dates = auditSeries.dates || [];
    if (!dates.length) return { startIndex: 0, endIndex: 0, dates: [] };

    const useFullSample = !Number.isFinite(Number(maxPoints)) || Number(maxPoints) <= 0 || dates.length <= Number(maxPoints);
    const startIndex = useFullSample ? 0 : Math.max(0, dates.length - Number(maxPoints));
    ...
    return {
      ...
      contextChangesBps: slice(auditSeries.contextChangesBps),
  ...
  computeEventStudy(series, config) {
    if (config.eventMode === 'price-extreme') return this.computePriceExtremeEventStudy(series, config);
    if (config.eventMode === 'level-change') return this.computeLevelChangeEventStudy(series, config);
    return this.computeDirectionalEventStudy(series, config);
  },
  ...
  computeLevelChangeEventStudy(series, config) {
    const { dates, closes, adjustedCloses, opens, contextCloses } = series;
    const targetSeries = adjustedCloses.every(Number.isFinite) ? adjustedCloses : closes;
    const { targetReturns, contextReturns } = this.computeSeriesReturns(targetSeries, contextCloses);
    const thresholdBps = Number(config.changeThresholdBps || config.threshold || 25);
    const thresholdPoints = thresholdBps / 100;
    const lookback = Math.max(1, Number(config.changeLookbackSessions || config.windowSessions || 1));
    const direction = config.changeDirection || (config.relation === 'opposite-direction' ? 'cut' : 'hike');
    const contextChangesBps = new Array(dates.length).fill(null);
    const thresholdLine = new Array(dates.length).fill(direction === 'cut' ? -thresholdBps : thresholdBps);
    const events = [];
    const baseline = [];
    const baselinePaths = [];
    const entryOffset = config.entryMode === 'next-open' ? 1 : 0;
    ...
    for (let signalIndex = lookback; signalIndex < dates.length; signalIndex++) {
      const priorLevel = contextCloses[signalIndex - lookback];
      const currentLevel = contextCloses[signalIndex];
      if (!Number.isFinite(priorLevel) || !Number.isFinite(currentLevel)) continue;
      const changePoints = currentLevel - priorLevel;
      const changeBps = changePoints * 100;
      contextChangesBps[signalIndex] = changeBps;
      const hit = direction === 'cut'
        ? changePoints <= -thresholdPoints
        : changePoints >= thresholdPoints;
      ...
      events.push({
        ...
        entryDate: dates[entryIndex],
        exitDate: dates[exitIndex],
        matchCount: Math.abs(changeBps),
        eventLabel: `${changeBps >= 0 ? '+' : ''}${PA.Fmt.number(changeBps, 0)} bps`,
        forwardReturn,
        targetReturn: targetReturns[signalIndex],
        contextReturn: contextReturns[signalIndex],
        contextLevel: currentLevel,
        contextChangeBps: changeBps,
        path
      });
      if (config.suppressOverlap) lastBlockedIndex = exitIndex;
    }
    ...
    if (!events.length) {
      throw new Error(`No qualifying ${direction === 'cut' ? 'rate-cut' : 'rate-hike'} events of at least ${thresholdBps} bps were found for this setup.`);
    }
  ...
  renderAuditChart(config, result) {
    ...
    if (!window.Plotly) {
      ...
      } else if (config.eventMode === 'level-change') {
        PA.Charts.multiLine('sl-chart-audit-fallback', audit.dates, [
          { label: `${contextLabel} change (bps)`, data: audit.contextChangesBps, color: '#f97316' },
          { label: 'Threshold', data: audit.thresholdLine, color: '#fbbf24' }
        ]);
      } else {
        PA.Charts.multiLine('sl-chart-audit-fallback', audit.dates, [
    ...
    if (config.eventMode === 'price-extreme') {
      ...
      const signalY = audit.visibleEvents.map(event => audit.contextChangesBps[event.localIndex]);
      traces.push({
        type: 'bar',
        x: audit.dates,
        y: audit.contextChangesBps,
        name: `${contextLabel} change (bps)`,
        marker: { color: audit.contextChangesBps.map(value => Number(value) >= 0 ? '#34d399' : '#f87171') },
        opacity: 0.72
      });
      traces.push({
        type: 'scatter',
      ...
      traces.push({
        ...
        name: 'Signal dates',
        marker: { color: '#f8fafc', size: 8, line: { color: '#f97316', width: 2 } },
        hovertemplate: '%{x}<br>Rate change: %{y:.0f} bps<extra></extra>'
      });
      traces.push({
        type: 'scatter',
        mode: 'lines',
  ...
  renderResults(config, result) {
    ...
    const contextLabel = ['price-extreme', 'level-change'].includes(config.eventMode) ? 'Trigger' : 'Context';
    const primaryEventColumn = config.eventMode === 'price-extreme' ? 'Trigger Event' : (config.eventMode === 'level-change' ? 'Rate Change' : 'Match Count');
      ...
      ? `
          <div class="strategy-table-wrap">
            <table class="data-table strategy-event-table">
              <thead>
                <tr><th>Mode</th><th class="right">Event Count</th><th class="right">Win Rate</th><th class="right">Average ${config.forwardSessions}D Return</th></tr>
              </thead>
    ...
    resultsDiv.innerHTML = `
      <div class="card">
        ...
        <div class="strategy-results-grid">
          ...
          <div class="strategy-kpi">
            <div class="strategy-kpi-label">2. Win Rate</div>
            <div class="strategy-kpi-value">${PA.Fmt.pct(requestedStats.value)}</div>
            <div class="strategy-kpi-detail">${PA.UI.escapeHtml(requestedStats.formula || trade.winFormula)}. Entry ${config.entryMode === 'next-open' ? 'next open' : 'signal close'}; close ${config.forwardSessions} sessions later. Baseline ${PA.Fmt.pct(requestedStats.baseline)} | edge ${PA.Fmt.pct(requestedStats.edge)}.</div>
          </div>
          <div class="strategy-kpi">
          ...
          <div class="strategy-kpi">
            <div class="strategy-kpi-label">Long / Short Target Check</div>
            ...
            <div class="strategy-kpi-detail">Long the target asset after every signal: ${PA.Fmt.pct(result.tradeCurve.longTotalReturn)} | short the target asset after every signal: ${PA.Fmt.pct(result.tradeCurve.shortTotalReturn)}. Compounded, not annualized.</div>
          </div>
          <div class="strategy-kpi">
      ...
      <div class="grid-2" style="margin-top:16px">
        <div class="card">
          ...
          <div class="strategy-note">
            ${PA.UI.escapeHtml(config.eventMode === 'level-change'
              ? 'This chart covers the full study window and plots every signal date, the trigger rate change in basis points, and the normalized target price so you can verify that the hike or cut really happened.'
              : config.eventMode === 'price-extreme'
                ? 'This chart covers the full study window and plots every signal date, the trigger level, its reference line, and the target normalized price so you can verify that the high or low really happened.'
                : 'This chart covers the full study window and plots every signal date, target and context 1D returns, and the rolling match count so you can verify that the trigger condition really fired.')}
          </div>
        ...
        <div class="card">
          ...
          <div class="strategy-note">Each step is one signal, not one day. This shows what buying the target asset after each signal versus shorting the target asset after each signal would have compounded to across the sample.</div>
    ...
    PA.Charts.multiLine('sl-chart-trades', forwardLabels, [
      { label: 'Long target after signal', data: result.tradeCurve.longCurve, color: '#4f8ff7' },
      { label: 'Short target after signal', data: result.tradeCurve.shortCurve, color: '#f97316' }
    ], { pct: true });
  }
};
