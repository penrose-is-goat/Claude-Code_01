...
PA.Dashboard = {
  ...
  heatmapModes: {
    ...
    spy: {
      label: 'SPY Holdings',
      title: 'SPY Holdings Heatmap',
      sourceTicker: 'SPY',
      dynamicHoldings: true,
      maxHoldings: 505,
      items: []
    },
    assets: {
  ...
  async loadHoldingUniverse(sourceTicker='SPY') {
    ...
    const mode = this.heatmapModes[this.heatmapMode] || {};
    const maxHoldings = Number(mode.maxHoldings || 75);
    ...
    const fallback = this.fallbackSpyHoldings().slice(0, maxHoldings);
    const note = `${symbol} holdings were unavailable from the backend fund profile, so this is a fallback top-holdings subset.`;
  ...
  async resolveUniverse() {
    ...
    if (mode.dynamicHoldings) {
      return this.loadHoldingUniverse(mode.sourceTicker || 'SPY');
    }
    return mode.items || [];
  },
  ...
  openHeatmapTicker(ticker) {
    ...
    if (symbol === 'SPY' && this.heatmapMode !== 'spy') {
      this.heatmapMode = 'spy';
      this.loadMarketOverview();
      return;
    }
  ...
  renderHeatmapChart(containerId, heatmapValues, maxAbs) {
    ...
    const leafItems = heatmapValues.map(item => {
      ...
      const explicitScale = Number(item.tileValue ?? item.sourceWeight);
      return {
        ...item,
        tileValue: Number.isFinite(explicitScale) && explicitScale > 0
          ? explicitScale
          : (Number.isFinite(assetScale) && assetScale > 0 ? assetScale : 1)
      };
    });
  ...
  async loadMarketOverview(options={}) {
    ...
    try {
      ...
      const [quotes, returnHistories] = await Promise.all([
        this.getDashboardQuotes(tickers, options),
        this.heatmapRange === '1D'
          ? Promise.resolve({})
          : this.getDashboardHistories(tickers, this.requiredHistoryRange(this.heatmapRange), options)
      ]);
      const quoteMap = {};
      (quotes || []).forEach(q => { quoteMap[q.symbol] = q; });
      ...
      const heatmapValues = universe.map(item => {
        ...
        return {
          ...item,
          quote,
          group: item.group || quote?.providerRaw?.info?.sector || 'Other',
          dailyReturn,
      ...
      container.innerHTML = `
        <div class="market-dashboard-stack">
          ...
          <div>
            ...
            <div class="market-note" style="margin-bottom:12px">Click <strong>SPY</strong> to drill into a holdings heatmap; click a stock tile to open the ticker page. Tile size uses fund weight, ETF net assets, or market cap when available.</div>
