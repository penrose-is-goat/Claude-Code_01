...
PA.Dashboard = {
  ...
  async loadCorrelationPanel(_tickers=[], renderKey='', options={}) {
    ...
    try {
      ...
      const matrices = groups.map(group => {
        ...
        const subtitle = summary?.strongest && summary?.weakest
          ? `Average ${PA.Fmt.ratio(summary.average)} | Highest ${summary.strongest.left}/${summary.strongest.right}: ${PA.Fmt.ratio(summary.strongest.corr)} | Lowest ${summary.weakest.left}/${summary.weakest.right}: ${PA.Fmt.ratio(summary.weakest.corr)}`
          : `${corrTickers.length} tickers with usable overlapping history`;
        return `
          <section class="correlation-matrix-panel">
            <div class="correlation-matrix-header">
              <strong>${PA.UI.escapeHtml(group.label)}</strong>
              <span>${PA.UI.escapeHtml(subtitle)}</span>
            </div>
            <div class="correlation-matrix-scroll">${matrixHtml}</div>
          </section>
        `;
      });

      panel.innerHTML = `
        <div class="market-note" style="margin-bottom:10px">Fixed 1Y daily-return matrices are shown on every dashboard view: Major Indexes, Total Market / Asset Classes, and S&P Sectors. They do not change into giant drilldown-specific matrices when you click into ETF holdings.</div>
        <div class="correlation-matrix-stack">${matrices.join('')}</div>
      `;
    } catch (error) {
      if (this.lastCorrelationKey !== renderKey) return;
