  ...
  async run() {
    ...
    try {
      ...
      this.tickers.forEach(t => {
        ...
        const cachedSummary = PA.Ticker.loadSummaryFromDb(t);
        const cachedSd = cachedSummary?.summaryDetail || {};
        const cachedKs = cachedSummary?.defaultKeyStatistics || {};
        rows.push({
          ticker: t,
          price: q.regularMarketPrice,
          change: q.regularMarketChangePercent,
          beta: hasAlignedRisk ? PA.Compute.beta(alignedReturns, alignedSpyReturns) : (q.beta ?? PA.Ticker.v(cachedKs.beta) ?? null),
          pe: q.trailingPE ?? PA.Ticker.v(cachedSd.trailingPE) ?? null,
          fwdPe: q.forwardPE ?? PA.Ticker.v(cachedKs.forwardPE) ?? PA.Ticker.v(cachedSd.forwardPE) ?? null,
          ...
          marketCap: q.marketCap ?? PA.Ticker.v(cachedSd.marketCap) ?? null,
          volume: q.regularMarketVolume,
          yield: q.trailingAnnualDividendYield ?? PA.Ticker.v(cachedSd.dividendYield) ?? null,
      ...
      const metrics = [
        ...
        ['Beta (1Y vs SPY)', r => PA.Fmt.ratio(r.beta)],
        ['P/E', r => PA.Fmt.ratio(r.pe)],
        ['Fwd P/E', r => PA.Fmt.ratio(r.fwdPe)],
        ['Delta (1Y)', r => PA.Fmt.ratio(r.delta, 3)],
        ['Gamma (1Y)', r => PA.Fmt.ratio(r.gamma, 4)],
        ['Alpha (1Y)', r => `<span class="${PA.Fmt.colorClass(r.alpha)}">${PA.Fmt.ratio(r.alpha, 4)}</span>`],
  ...
  async init() {
    ...
    container.innerHTML = `
      <div style="padding:10px;color:var(--text-muted)">
        Market overview is manual in ${PA.Config.APP_VERSION} to conserve Twelve Data credits.
