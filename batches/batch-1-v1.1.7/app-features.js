...
PA.Ticker = {
  ...
  async lookup(ticker, options={}) {
    ...
    try {
      if (!options.forceLive) {
        const cached = this.loadFreshFromDb(ticker, this.currentRange);
        const cachedHasFundamentals = cached && (
          this.hasFundamentalData(cached.summary) ||
          cached.quote?.marketCap != null ||
          cached.quote?.trailingPE != null ||
          cached.quote?.epsTrailingTwelveMonths != null
        );
        if (cached && cachedHasFundamentals) {
          this.current = cached;
          this.render(true);
          PA.UI.toast('Loaded cached data', 'info');
          return;
        }
      }

      const cachedSummary = this.loadSummaryFromDb(ticker);
      const shouldFetchSummary = options.forceLive || !this.hasFundamentalData(cachedSummary);
      const useLiveQuote = PA.API.shouldUseLiveQuote(options.forceLive);
      const [quotes, history] = await Promise.allSettled([
        useLiveQuote ? PA.API.getQuote([ticker]) : Promise.resolve([]),
        PA.API.getHistory(ticker, this.currentRange)
      ]);
      const summaryResult = await Promise.allSettled([
        shouldFetchSummary ? PA.API.getQuoteSummary(ticker) : Promise.resolve(cachedSummary)
      ]);
      const hist = history.status === 'fulfilled' ? history.value : null;
      const summary = summaryResult[0]?.status === 'fulfilled'
        ? (summaryResult[0].value || cachedSummary || {})
        : (cachedSummary || {});
      ...
      this.current = { ticker, quote: mergedQuote, hist, summary };
      this.render();
      try {
      ...
      if (summaryResult[0]?.status === 'rejected' && !this.hasFundamentalData(summary)) {
        PA.UI.toast('Live price loaded, but fundamentals were unavailable for this lookup', 'error');
      }
    } catch(e) {
      PA.UI.renderError(
        'ticker-content',
  ...
  render(isCached=false) {
    ...
    const oneDayReturn = this.calculateReturn(hData.prices, 1);
    const oneMonthReturn = this.calculateReturn(hData.prices, 21);
    const threeMonthReturn = this.calculateReturn(hData.prices, 63);
    const oneYearReturn = this.calculateReturn(hData.prices, 252);
    ...
    const metricCards = isEtf
      ? [
          metricCard('Beta (1Y vs SPY)', '<span id="metric-beta">--</span>'),
          metricCard('Expense Ratio', expenseRatio != null ? PA.Fmt.pct(expenseRatio, 3) : 'N/A'),
          metricCard('Net Assets', netAssets ? '$' + PA.Fmt.compact(netAssets) : 'N/A'),
          metricCard('Yield', yld != null ? PA.Fmt.pct(yld) : 'N/A'),
          metricCard('Turnover', portfolioTurnover != null ? PA.Fmt.pct(portfolioTurnover) : 'N/A'),
    ...
    const html = `
      <div class="ticker-header">
        ...
        <span class="ticker-change ${changeClass}">${changeSign}${change.toFixed(2)} (${changeSign}${changePct.toFixed(2)}%) <span style="font-size:0.72rem;opacity:0.8">1D</span></span>
      </div>
      ...
      <div style="color:var(--text-muted);font-size:0.78rem;margin:-8px 0 16px">
        Displayed price move = 1D vs previous close. Beta, alpha, delta, and gamma use trailing 1Y aligned against SPY.
      </div>
    ...
    const stats = isEtf
      ? [
          ...
          ['Beta (1Y vs SPY)', '<span id="stat-beta">--</span>'],
          ...
          ['Expense Ratio', expenseRatio != null ? PA.Fmt.pct(expenseRatio, 3) : 'N/A'],
          ['Portfolio Turnover', portfolioTurnover != null ? PA.Fmt.pct(portfolioTurnover) : 'N/A'],
          ['Distribution Yield', yld != null ? PA.Fmt.pct(yld) : 'N/A'],
      ...
      : [
          ...
          ['Reported Beta (Provider)', providerBeta != null ? PA.Fmt.ratio(providerBeta) : 'N/A']
        ];
    ...
    if (hData.prices.length > 30) {
      if (isCached) {
        this.loadGreeksFromDb(ticker);
        this.computeGreeks(ticker, this.currentRange === '1Y' ? hData : null);
      } else {
        this.computeGreeks(ticker, hData);
      }
    } else if (providerBeta != null) {
      this.updateGreekDisplays({ beta: providerBeta });
    }
  },
  ...
  calculateReturn(prices, periods) {
    if (!Array.isArray(prices) || prices.length <= periods) return null;
    const start = prices[prices.length - periods - 1];
