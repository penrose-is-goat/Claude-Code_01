...
PA.App = {
  async init() {
    try {
      ...
      document.getElementById('status-text').textContent = PA.API.getApiKey()
        ? 'Ready (Twelve Data + Alpha Vantage fundamentals)'
  ...
  injectApiSettings() {
    ...
    settingsGrid.insertAdjacentHTML('afterbegin', `
      <div class="card" id="td-settings-card">
        <div class="card-title">Market Data APIs</div>
        <div class="form-group">
          <label class="form-label">Twelve Data Key (live price + history)</label>
          <input class="form-input" id="td-api-key" type="password" placeholder="Paste your Twelve Data API key">
        </div>
        <div class="form-group" style="margin-top:12px">
          <label class="form-label">Alpha Vantage Key (fundamentals)</label>
          <input class="form-input" id="av-api-key" type="password" placeholder="Paste your Alpha Vantage API key">
        </div>
        <div style="display:flex;gap:8px;align-items:center;margin-top:12px">
          <button class="btn btn-primary" onclick="PA.App.saveApiKey()">Save Keys</button>
          <button class="btn" onclick="PA.App.testApiKey()">Test APIs</button>
          <button class="btn" onclick="PA.App.clearApiKey()">Clear</button>
        </div>
        <div style="margin-top:10px;font-size:0.8rem;color:var(--text-muted)">
          Twelve Data powers live quotes/history. Alpha Vantage powers stock fundamentals plus ETF profile data such as net assets, expense ratio, turnover, and distribution yield.
        </div>
        <div style="margin-top:8px;font-size:0.8rem;color:var(--text-muted)">
          Free keys are rate-limited. In Batch 1 v1.1.7, computed beta/alpha/delta/gamma use trailing 1Y history aligned to SPY, while provider beta is shown separately in Additional Stats.
        </div>
    ...
    const avInput = document.getElementById('av-api-key');
    if (avInput) avInput.value = PA.API.getAlphaVantageKey();
    this.refreshDiagnostics();
  },
  ...
  refreshDiagnostics() {
    ...
    container.innerHTML = `
      ...
      <div><strong>Twelve Data Key:</strong> ${diagnostics.hasApiKey ? 'Detected' : 'Missing'}</div>
      <div><strong>Alpha Vantage Key:</strong> ${diagnostics.hasFundamentalsKey ? 'Detected' : 'Missing'}</div>
  ...
  saveApiKey() {
    const tdInput = document.getElementById('td-api-key');
    const avInput = document.getElementById('av-api-key');
    PA.API.setApiKey(tdInput?.value || '');
    PA.API.setAlphaVantageKey(avInput?.value || '');
    PA.API.cache.clear();
    document.getElementById('status-text').textContent = PA.API.getApiKey()
      ? 'Ready (Twelve Data + Alpha Vantage fundamentals)'
      : 'Add API keys in Settings';
    PA.UI.toast('API keys saved', 'success');
    this.refreshDiagnostics();
    PA.Dashboard.init();
  },
  ...
  async testApiKey() {
    try {
      ...
      if (pe == null) {
        throw new Error('Fundamentals test returned no NVDA P/E data.');
      }
      PA.UI.toast(`API test passed: NVDA ${PA.Fmt.currency(quote.regularMarketPrice)} | P/E ${PA.Fmt.ratio(pe)}`, 'success');
    } catch (e) {
  ...
  clearApiKey() {
    PA.API.setApiKey('');
    PA.API.setAlphaVantageKey('');
    PA.API.cache.clear();
    const input = document.getElementById('td-api-key');
    if (input) input.value = '';
    const avInput = document.getElementById('av-api-key');
    if (avInput) avInput.value = '';
    document.getElementById('status-text').textContent = 'Add API keys in Settings';
    PA.UI.toast('API keys cleared', 'info');
    this.refreshDiagnostics();
  },
