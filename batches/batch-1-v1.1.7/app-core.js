...
PA.API = {
  ...
  getAlphaVantageKey() {
    const key = localStorage.getItem('pa_alpha_vantage_key') || PA.Config.DEFAULT_ALPHA_VANTAGE_KEY || '';
    return key.trim();
  },
  ...
  setAlphaVantageKey(key) {
    const cleaned = (key || '').trim();
    if (cleaned) {
      localStorage.setItem('pa_alpha_vantage_key', cleaned);
    } else {
  ...
  getDiagnostics() {
    return {
      provider: 'Twelve Data + Alpha Vantage fundamentals',
      version: PA.Config.APP_VERSION,
      hasApiKey: Boolean(this.getApiKey()),
      hasFundamentalsKey: Boolean(this.getAlphaVantageKey()),
  ...
  shouldUseLiveQuote(forceLive=false) {
    return Boolean(forceLive) || this.isUsMarketOpenNow();
  },

  requireAlphaVantageKey() {
    const key = this.getAlphaVantageKey();
    if (!key) {
      throw new Error('Missing Alpha Vantage fundamentals key. Add it in Settings first.');
    }
    return key;
  },
  ...
  async getQuote(tickers) {
    ...
    if (unresolved.length) {
      throw this.recordError(
        new Error(`No quote returned for ${unresolved.join(', ')} from Twelve Data.`),
  ...
  async getQuoteSummary(ticker) {
    ...
    const cached = this.getCached(key, this.SUMMARY_TTL);
    if (cached) return cached;
    let result = null;
    const security = PA.DB?.selectOne
      ? PA.DB.selectOne(`SELECT asset_type FROM securities WHERE ticker=?`, [symbol])
      : null;
    const shouldUseEtfProfileFirst = security?.asset_type === 'etf';
    if (shouldUseEtfProfileFirst) {
      const etfRaw = await this.fetchAlphaVantageJson({
        function: 'ETF_PROFILE',
        symbol
      });
      result = this.normalizeAlphaVantageEtfProfile(etfRaw, symbol);
    } else {
      const raw = await this.fetchAlphaVantageJson({
        function: 'OVERVIEW',
        symbol
      });
      if (raw?.Symbol) {
        result = this.normalizeAlphaVantageOverview(raw);
      } else {
        const etfRaw = await this.fetchAlphaVantageJson({
          function: 'ETF_PROFILE',
          symbol
    ...
    if (result?.fundProfile?.netAssets != null) {
      result.company = {
        ...(result.company || {}),
        industry: 'ETF'
      };
  ...
  async getHistories(tickers, rangeKey='1Y') {
    ...
    if (unresolved.length) {
      throw this.recordError(
        new Error(`No price history returned for ${unresolved.join(', ')} from Twelve Data.`),
        { endpoint: 'time_series', symbols: unresolved, rangeKey }
      );
    }
