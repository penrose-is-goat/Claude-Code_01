...
PA.Ticker = {
    ...
    try {
      ...
      if (!hist || !quote) {
        ...
        throw PA.API.recordError(
          new Error(`Live quote/history was incomplete for ${ticker}. Quote returned: ${Boolean(quote)}. History returned: ${Boolean(hist)}.`),
          { scope: 'ticker.lookup', ticker, quoteReturned: Boolean(quote), historyReturned: Boolean(hist) }
        );
      }

      const hData = PA.API.parseHistory(hist);
      const mergedQuote = PA.API.applySummaryToQuote(quote, summary);
  ...
  loadGreeksFromDb(ticker) {
    ...
    setEl('metric-beta', greeks.beta, 2);
    setEl('metric-delta', greeks.delta, 3);
    setEl('metric-gamma', greeks.gamma, 4);
    setEl('metric-alpha', greeks.alpha, 4);
  },
