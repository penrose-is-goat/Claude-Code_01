...
- Goal: stable live quote/history lookup, portfolio builder, compare, and backtest foundation with cache-first behavior for free-tier testing plus clearer diagnostics

## Current Status

What Batch 1 v1.1.3 is meant to handle well:
...
7. Footer shows `Batch 1 v1.1.3`

## v1.1.3 Fixes

- Replaced fragile raw `e.message` rendering with normalized error handling
- Added a Settings diagnostics panel for last request / last provider error
- Improved partial-response handling so incomplete quote/history loads no longer pretend the ticker was simply "not found"
